...
describe("RESTORE-100 signing-key entrypoint contract", () => {
  it("creates a single owner-only random key outside backups without following a final-path symlink", () => {
    ...
    expect(entrypoint).toContain('"600:root:root"');
    expect(entrypoint).toContain('secure_persistent_path file "$BACKUP_SIGNING_KEY_FILE" 0 0 0600');
    expect(entrypoint).toContain('"$BACKUP_SIGNING_KEY_FILE" 0 0 "$RUNTIME_API_BACKUP_SIGNING_KEY_FILE"');
    expect(entrypoint).toContain('"$BACKUP_SIGNING_KEY_FILE" 0 0 "$RUNTIME_RESTORE_BACKUP_SIGNING_KEY_FILE"');
    ...
    expect(entrypoint).toContain('secure_persistent_path directory /app/.ingenium "$RESTORE_UID" "$RESTORE_DATA_GID" 3770');
    expect(entrypoint).toContain('PROJECTS_DIR="/app/.ingenium/projects"');
    expect(entrypoint).toContain('secure_persistent_path directory "$PROJECTS_DIR" "$API_UID" "$API_GID" 0700');
    expect(entrypoint).toContain('BACKUPS_DIR="/app/.ingenium/backups"');
    expect(entrypoint).toContain('secure_persistent_path directory "$BACKUPS_DIR" "$API_UID" "$RESTORE_DATA_GID" 2770');
    expect(entrypoint).toContain('RESTORE_STAGING_DIR="${INGENIUM_RESTORE_STAGING_DIR:-/app/.ingenium/restore-staging}"');
    expect(entrypoint).toContain('secure_persistent_path directory "$RESTORE_STAGING_DIR" "$API_UID" "$RESTORE_DATA_GID" 2770');
    expect(entrypoint).toContain('if (metadata.isSymbolicLink() || !metadata.isDirectory()) fail();');
