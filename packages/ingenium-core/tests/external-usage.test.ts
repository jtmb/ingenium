...
describe("external usage ledger", () => {
  it("accepts an API-verified native binding without consulting coordination sessions", () => {
    const nativeProject = createProject("ingenium-native-usage").id;

    const result = ingestExternalUsage(nativeProject, worktreeId, input, nativeValidation);
  ...
  it("does not misclassify a storage failure as a foreign binding or retain a partial event", () => {
    ...
    expect(() => ingestExternalUsage(projectId, worktreeId, input, nativeValidation)).toThrow("storage unavailable");
  ...
  it("persists one immutable contribution across replay and DB restart with unknown cost and zero output", () => {
    const first = ingestExternalUsage(projectId, worktreeId, input, nativeValidation);
    expect(first.created).toBe(true);
    expect(ingestExternalUsage(projectId, worktreeId, input, nativeValidation)).toEqual({ ...first, created: false });
    resetDbForTest();
    expect(ingestExternalUsage(projectId, worktreeId, input, nativeValidation)).toEqual({ ...first, created: false });
    ...
    expect(() => ingestExternalUsage(projectId, worktreeId, { ...input, inputTokens: 99 }, nativeValidation)).toThrow("SOURCE_CONFLICT");
  ...
  it("rejects foreign bindings, non-assistant/unfinished payloads, extras, unsafe identifiers and invalid numbers", () => {
    expect(() => ingestExternalUsage(projectId, "foreign", input, nativeValidation)).toThrow("BINDING_REJECTED");
    expect(() => ingestExternalUsage(projectId, worktreeId, input)).toThrow("BINDING_REJECTED");
    for (const override of [{ role: "user" }, { role: "tool" }, { completedAt: undefined }, { text: "secret-canary" },
      ...
      expect(() => ingestExternalUsage(projectId, worktreeId, { ...input, ...override }, nativeValidation)).toThrow("INVALID_USAGE_INPUT");
    }
    expect(getUsageSummary(projectId, range).totals.requests).toBe(0);
  });

  it("preserves API-local mutable step sync independently of immutable external message usage", () => {
    const external = ingestExternalUsage(projectId, worktreeId, { ...input, costAmount: 0 }, nativeValidation);
    ...
    expect(ingestExternalUsage(projectId, worktreeId, { ...input, costAmount: 0 }, nativeValidation).created).toBe(false);
