-- AUTH-109 audience-bound browser launch and runtime sessions.
BEGIN IMMEDIATE;

CREATE TABLE runtime_browser_generations (
  runtime_id TEXT PRIMARY KEY REFERENCES runtime_instances(id) ON DELETE RESTRICT,
  generation INTEGER NOT NULL DEFAULT 0 CHECK(generation >= 0),
  updated_at TEXT NOT NULL CHECK(length(updated_at) BETWEEN 1 AND 64)
);

CREATE TABLE runtime_browser_launch_tickets (
  ...
  origin TEXT NOT NULL CHECK(length(origin) BETWEEN 9 AND 512 AND substr(origin, 1, 8) = 'https://'),
...
INSERT INTO runtime_browser_generations (runtime_id, generation, updated_at)
SELECT id, security_epoch, datetime('now') FROM runtime_instances;
