-- Migration 017: Fix personality_traits FK to reference the current observations table.
-- Migration 015 renamed observations -> observations_old and created a new observations table.
-- The FK in personality_traits (exemplar_observation_id -> observations(id)) was defined in
-- migration 008 and may reference a stale table reference after the rename cycle.
-- Rebuild the table so the FK explicitly points to the current observations table.

ALTER TABLE personality_traits RENAME TO personality_traits_old;
...
CREATE TABLE IF NOT EXISTS personality_traits (
    ...
    exemplar_observation_id INTEGER REFERENCES observations(id),
...
INSERT INTO personality_traits SELECT * FROM personality_traits_old;
DROP TABLE personality_traits_old;
...
-- Recreate the personality profile view (harmless DROP/CREATE to ensure it references the new table)
