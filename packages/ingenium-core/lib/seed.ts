...
export function seedSkills(projectId: string, skillsDir: string): number {
  let count = 0;
  execTransaction(() => {
    ...
    for (const entry of entries) {
      ...
      if (existsSync(metaPath)) {
        try {
          ...
          if (Array.isArray(meta.tags)) metaTags = meta.tags.join(",");
          if (meta.alwaysApply === true) metaAlwaysApply = 1;
        } catch {}
      }
      ...
      const result = db.prepare(
        `INSERT OR IGNORE INTO skills (id, project_id, name, description, content, tags, always_apply, file_tree, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ).run(id, projectId, name, description, content, metaTags, metaAlwaysApply, fileTree, now, now);
...
export function seedPlugins(projectId: string, pluginsDir: string): number {
  let count = 0;
  execTransaction(() => {
    const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
    ...
    for (const entry of entries) {
      ...
      const id = randomUUID();
      db.prepare(
        `INSERT OR IGNORE INTO plugins (id, project_id, name, file_path, enabled, source_content, created_at, updated_at)
         VALUES (?, ?, ?, ?, 1, ?, ?, ?)`
      ).run(id, projectId, name, entry.name, sourceContent, now, now);
...
export function seedAgents(projectId: string, agentsDir: string): number {
  let count = 0;
  execTransaction(() => {
    const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
