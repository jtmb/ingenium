  ...
  execTransaction(() => {
    ...
    for (const entry of entries) {
      ...
      const result = db.prepare(
        `INSERT OR IGNORE INTO skills (id, project_id, name, description, content, tags, always_apply, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ).run(id, projectId, name, description, content, metaTags, metaAlwaysApply, now, now);
