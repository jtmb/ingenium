...
/** Raised when a checkpoint has made a source part of immutable history. */
export class RagSourceImmutableError extends Error {
  constructor() {
    super("RAG source is referenced by an immutable context checkpoint");
...
/**
 * Checkpoint provenance must keep pointing at the exact indexed document. The
 * database trigger is the final authority; this preflight gives API callers a
 * stable error before a write is attempted.
 */
function assertSourceMutable(db: ReturnType<typeof getDb>, sourceId: string): void {
  const linked = db.prepare(
    "SELECT 1 FROM context_checkpoint_rag_sources WHERE rag_source_id = ? LIMIT 1",
  ).get(sourceId);
  if (linked) throw new RagSourceImmutableError();
}
 ...
 * This lets context uploads make source metadata, chunks, embeddings, and
