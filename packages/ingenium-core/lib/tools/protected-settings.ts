...
export type OAuthClientSecretResult =
  | { status: "ok"; metadata: OAuthClientSecretMetadata }
  | { status: "vault_unavailable"; metadata: OAuthClientSecretMetadata }
  | { status: "legacy_conflict"; metadata: OAuthClientSecretMetadata }
  | { status: "invalid"; metadata: OAuthClientSecretMetadata };

export type OAuthClientSecretLifecycleMigration =
  | { status: "completed"; results: OAuthClientSecretResult[] }
  | { status: "no_active_global"; results: [] }
...
export function isOAuthClientSecretKey(value: unknown): value is OAuthClientSecretKey {
  return typeof value === "string" && (OAUTH_CLIENT_SECRET_KEYS as readonly string[]).includes(value);
}
...
function vaultItemName(key: OAuthClientSecretKey): string {
  return key === "oauth_gmail_client_secret"
    ? "OAuth Client Secret: Gmail"
    : "OAuth Client Secret: Outlook";
}

function nextVaultItemName(projectId: string, key: OAuthClientSecretKey): string {
  const name = vaultItemName(key);
  const existing = getDb(dbPath()).prepare(
    "SELECT 1 FROM vault_items WHERE project_id = ? AND name = ?",
  ).get(projectId, name);
...
function getReference(projectId: string, key: OAuthClientSecretKey): { vault_item_id: string } | undefined {
  return getDb(dbPath()).prepare(
    `SELECT ps.vault_item_id
     FROM protected_settings ps
     INNER JOIN vault_items vi ON vi.id = ps.vault_item_id
     WHERE ps.project_id = ? AND ps.key = ? AND vi.project_id = ? AND vi.access_policy <> ?`,
  ).get(projectId, key, projectId, DELETED_POLICY) as { vault_item_id: string } | undefined;
}

/** A soft-deleted item has no usable secret and may leave a stale mapping after an interrupted clear. */
function getInactiveReference(projectId: string, key: OAuthClientSecretKey): { vault_item_id: string } | undefined {
  return getDb(dbPath()).prepare(
    `SELECT ps.vault_item_id
     FROM protected_settings ps
     INNER JOIN vault_items vi ON vi.id = ps.vault_item_id
...
function removeInactiveReference(projectId: string, key: OAuthClientSecretKey): boolean {
  const inactive = getInactiveReference(projectId, key);
  if (!inactive) return true;
  try {
    execTransaction(() => {
      getDb(dbPath()).prepare(
        "DELETE FROM protected_settings WHERE project_id = ? AND key = ? AND vault_item_id = ?",
      ).run(projectId, key, inactive.vault_item_id);
    });
...
function getLegacyValue(projectId: string, key: OAuthClientSecretKey): string | undefined {
...
function deleteLegacyValue(projectId: string, key: OAuthClientSecretKey): void {
  execTransaction(() => {
...
function metadata(projectId: string, key: OAuthClientSecretKey): OAuthClientSecretMetadata {
  const isSet = Boolean(getReference(projectId, key) || getLegacyValue(projectId, key)?.trim());
  return { isSet, masked: isSet };
}

function emptyMetadata(): OAuthClientSecretMetadata {
  return { isSet: false, masked: false };
}

function protectedValueMatches(projectId: string, key: OAuthClientSecretKey, expected: string): boolean {
...
function writeProtectedValue(projectId: string, key: OAuthClientSecretKey, value: string): boolean {
  if (vault.isSealed()) return false;

  let createdItemId: string | undefined;
  try {
    const existing = getReference(projectId, key);
    if (existing) {
      if (!vault.getItemMetadata(projectId, existing.vault_item_id)) return false;
      vault.updateItem(projectId, existing.vault_item_id, value);
      return protectedValueMatches(projectId, key, value);
    }

    if (!removeInactiveReference(projectId, key)) return false;

    createdItemId = vault.createItem(projectId, nextVaultItemName(projectId, key), "oauth", value);
    if (!createdItemId || createdItemId === "Vault is sealed") return false;

    const mappingCreated = execTransaction(() => {
      const result = getDb(dbPath()).prepare(
        `INSERT INTO protected_settings (project_id, key, vault_item_id, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?)
         ON CONFLICT(project_id, key) DO NOTHING`,
      ).run(projectId, key, createdItemId, new Date().toISOString(), new Date().toISOString());
...
function clearProtectedValue(projectId: string, key: OAuthClientSecretKey): boolean {
  if (vault.isSealed()) return false;
  const existing = getReference(projectId, key);
  if (!existing) {
    try {
  ...
  try {
    ...
    if (vault.getItemMetadata(projectId, existing.vault_item_id) !== null) return false;

    execTransaction(() => {
      getDb(dbPath()).prepare(
        "DELETE FROM protected_settings WHERE project_id = ? AND key = ? AND vault_item_id = ?",
      ).run(projectId, key, existing.vault_item_id);
    });
    checkpointAfterWrite();
    deleteLegacyValue(projectId, key);
    return true;
 ...
 */
export function migrateLegacyOAuthClientSecret(
  projectId: string,
  ...
  try {
    ...
    if (!legacy?.trim()) return { status: "ok", metadata: metadata(projectId, key) };
    if (vault.isSealed()) return { status: "vault_unavailable", metadata: metadata(projectId, key) };

    const existing = getReference(projectId, key);
    if (existing) {
      const protectedValue = vault.decryptItem(projectId, existing.vault_item_id);
      if (protectedValue === null) return { status: "vault_unavailable", metadata: metadata(projectId, key) };
      if (protectedValue !== legacy) return { status: "legacy_conflict", metadata: metadata(projectId, key) };
      deleteLegacyValue(projectId, key);
...
export function migrateLegacyOAuthClientSecrets(projectId: string): OAuthClientSecretResult[] {
...
export function migrateLegacyOAuthClientSecretsForActiveGlobalProject(): OAuthClientSecretLifecycleMigration {
  try {
    ...
    const outcomes = results.reduce<Record<OAuthClientSecretResult["status"], number>>((counts, result) => {
...
export function getOAuthClientSecret(projectId: string, key: OAuthClientSecretKey): string | undefined {
  if (!isActiveGlobalProjectId(projectId)) return undefined;
  if (vault.isSealed()) return undefined;
  const reference = getReference(projectId, key);
...
export function getOAuthClientSecretMetadata(projectId: string, key: OAuthClientSecretKey): OAuthClientSecretMetadata {
  if (!isActiveGlobalProjectId(projectId)) return emptyMetadata();
 ...
 */
export function updateOAuthClientSecret(
  projectId: string,
  key: OAuthClientSecretKey,
  action: OAuthClientSecretAction,
  value?: string,
): OAuthClientSecretResult {
  if (!isActiveGlobalProjectId(projectId)) {
    return { status: "invalid", metadata: emptyMetadata() };
  }
  if (action === "preserve") return migrateLegacyOAuthClientSecret(projectId, key);

  if (action === "replace") {
    if (!value?.trim()) return { status: "invalid", metadata: metadata(projectId, key) };
