 ...
 */
export function createSource(projectId, title, metadata = "{}") {
    const id = randomUUID();
    const db = getDb(dbPath());
    execTransaction(() => {
        db.prepare(`INSERT INTO rag_sources (id, project_id, title, source_type, metadata, created_at, updated_at)
       VALUES (?, ?, ?, 'text', ?, datetime('now'), datetime('now'))`).run(id, projectId, title, metadata);
    });
    checkpointAfterWrite();
    const row = db.prepare("SELECT * FROM rag_sources WHERE id = ?").get(id);
 ...
 */
export function ingestChunks(sourceId, chunks) {
    const db = getDb(dbPath());
    execTransaction(() => {
        const insert = db.prepare(`INSERT INTO rag_chunks (id, source_id, chunk_index, content, token_count, heading_path)
       VALUES (?, ?, ?, ?, ?, ?)`);
        const insertEmbedding = db.prepare(`INSERT INTO rag_embeddings (chunk_id, embedding, model_id, dimensions, created_at)
...
function replaceSourceContentInTransaction(db, sourceId, content, chunks, options) {
...
export function ingestCanonicalSource(projectId, title, content, options = {}) {
    const db = getDb(dbPath());
    const sourcePath = options.sourcePath ?? null;
    const hash = sha256(content);
    const existing = sourcePath ? db.prepare("SELECT * FROM rag_sources WHERE project_id = ? AND source_path = ?").get(projectId, sourcePath) : undefined;
    ...
    checkpointAfterWrite();
    return db.prepare("SELECT * FROM rag_sources WHERE id = ?").get(id);
}
/**
 * BM25 full-text search across all sources in a project.
 */
export function searchChunks(projectId, query, limit = 20, includeGlobal = true) {
    const sanitized = sanitizeFts5Query(query);
    if (!sanitized)
        return [];
    const globalProjectId = includeGlobal ? getGlobalProject()?.id ?? null : null;
    return getDb(dbPath()).prepare(`SELECT c.rowid AS _rowid, c.id, c.source_id, c.chunk_index, c.content, c.token_count,
     ...
     INNER JOIN rag_sources s ON s.id = c.source_id
      ...
      LIMIT ?`).all(projectId, globalProjectId, globalProjectId, sanitized, Math.max(1, limit));
}
/**
 ...
 */
export function ingestSource(projectId, name, content) {
    return rowToSource(ingestCanonicalSource(projectId, name, content));
}
/**
 * List all sources for a project.
 */
export function listSources(projectId, limit = 50, offset = 0) {
    const db = getDb(dbPath());
    const safeLimit = Math.min(Math.max(Math.trunc(limit), 1), 100);
    const safeOffset = Math.max(Math.trunc(offset), 0);
    const total = db.prepare("SELECT count(*) AS total FROM rag_sources WHERE project_id = ?").get(projectId).total;
    const data = db.prepare("SELECT * FROM rag_sources WHERE project_id = ? ORDER BY updated_at DESC, id DESC LIMIT ? OFFSET ?").all(projectId, safeLimit, safeOffset);
...
export function indexConfiguredDocs(globalProjectId, root = process.env.INGENIUM_DOCS_ROOT) {
    if (!root)
        throw new Error("INGENIUM_DOCS_ROOT must be configured");
    const rootReal = realpathSync(root);
    const docsRoot = realpathSync(resolve(rootReal, "docs"));
    if (!docsRoot.startsWith(`${rootReal}${sep}`))
        throw new Error("configured docs root escapes repository root");
    const files = [];
    ...
    for (const file of files.sort()) {
        ...
        if (!real.startsWith(`${docsRoot}${sep}`) && real !== docsRoot)
            continue;
        const path = `docs/${relative(docsRoot, real).split(sep).join("/")}`;
        active.add(path);
        const content = readFileSync(real, "utf8");
        const old = getDb(dbPath()).prepare("SELECT source_hash FROM rag_sources WHERE project_id = ? AND source_path = ?").get(globalProjectId, path);
        if (old?.source_hash === sha256(content)) {
            unchanged++;
            continue;
        }
        ingestCanonicalSource(globalProjectId, basename(file), content, { sourceType: "file", sourcePath: path, metadata: { kind: "file", repositoryPath: path, provenance: "configured-docs-root" } });
        indexed++;
    }
    const stale = getDb(dbPath()).prepare("SELECT id, source_path FROM rag_sources WHERE project_id = ? AND source_type = 'file' AND source_path LIKE 'docs/%'").all(globalProjectId);
    let deleted = 0;
    for (const source of stale)
        if (!active.has(source.source_path)) {
...
export function indexPublishedDoc(page) {
    const db = getDb(dbPath());
    ...
    if (page.status !== "published") {
        const source = db.prepare("SELECT id FROM rag_sources WHERE project_id = ? AND source_path = ?").get(global.id, sourcePath);
        if (source)
            deleteSource(source.id);
        return;
    }
 ...
 */
export function updateChunkEmbedding(chunkId, embedding, sourceId) {
    const db = getDb(dbPath());
    const buffer = embeddingBuffer(embedding);
    // Find the UUID by chunk_index (filtered by source_id if provided)
    let chunk;
    if (sourceId) {
        chunk = db.prepare("SELECT id FROM rag_chunks WHERE chunk_index = ? AND source_id = ?").get(chunkId, sourceId);
    }
    else {
        chunk = db.prepare("SELECT id FROM rag_chunks WHERE chunk_index = ?").get(chunkId);
    }
    if (!chunk)
        return;
    db.prepare(`INSERT INTO rag_embeddings (chunk_id, embedding, model_id, dimensions, created_at)
     VALUES (?, ?, 'ingenium-ngram-v1', ?, datetime('now'))
     ON CONFLICT(chunk_id) DO UPDATE SET
       embedding = excluded.embedding,
       model_id = excluded.model_id,
       dimensions = excluded.dimensions,
       created_at = excluded.created_at`).run(chunk.id, buffer, embedding.length);
}
/**
 * Hybrid search: fuses BM25 FTS scores with vector similarity.
 * Falls back to FTS-only when no embeddings are stored.
 */
export function hybridSearch(projectId, query) {
    if (!query.trim())
        return [];
    const db = getDb(dbPath());
    const sanitized = sanitizeFts5Query(query);
    // Get FTS results
    const ftsResults = sanitized ? db.prepare(`SELECT c.id AS chunk_uuid, c.chunk_index, c.source_id, c.content, c.heading_path,
     ...
     ORDER BY rank
     LIMIT 20`).all(projectId, sanitized) : [];
    const queryEmb = generateEmbedding(query);
    let maxRank = 1;
    for (const r of ftsResults) {
        if (Math.abs(r.rank) > maxRank)
            maxRank = Math.abs(r.rank);
    }
    const candidates = db.prepare(`SELECT c.id AS chunk_uuid, c.chunk_index, c.source_id, c.content, c.heading_path,
    ...
    const combined = candidates.map((candidate) => {
        const stored = Array.from(new Float32Array(candidate.embedding.buffer, candidate.embedding.byteOffset, candidate.embedding.byteLength / 4));
    ...
    for (const result of ftsResults) {
        if (embeddedIds.has(result.chunk_uuid))
            continue;
