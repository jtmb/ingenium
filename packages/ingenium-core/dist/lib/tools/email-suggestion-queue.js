 ...
 */
export function enqueueSuggestionJob(accountId, folder, uid) {
    const result = execTransaction(() => {
        ...
        const existing = db.prepare("SELECT 1 FROM email_suggestions WHERE account_id = ? AND folder = ? AND uid = ?").get(accountId, folder, uid);
        if (existing) {
            return false;
        }
        // Upsert with ON CONFLICT DO NOTHING — skip if already queued
        const result = db.prepare(`INSERT INTO email_suggestion_queue (account_id, folder, uid, created_at, attempts, next_attempt_at)
       VALUES (?, ?, ?, datetime('now'), 0, datetime('now'))
       ON CONFLICT(account_id, folder, uid) DO NOTHING`).run(accountId, folder, uid);
...
export function dequeueSuggestionJob() {
    const db = getDb(dbPath());
    const row = db.prepare(`SELECT id, account_id, folder, uid
...
export function markJobFailed(jobId, error) {
    execTransaction(() => {
        ...
        const row = db.prepare("SELECT attempts FROM email_suggestion_queue WHERE id = ?").get(jobId);
        if (!row) {
        ...
        if (attempts >= 5) {
            ...
            db.prepare("DELETE FROM email_suggestion_queue WHERE id = ?").run(jobId);
            logger.warn("email-suggestion-queue", `Job ${jobId} failed 5 times — deleted from queue. Last error: ${error}`);
            return;
        }
        // Exponential backoff: 30s, 60s, 120s, 300s, 600s
        const delays = [30, 60, 120, 300, 600];
        const delaySec = delays[attempts - 1] ?? 600;
        db.prepare(`UPDATE email_suggestion_queue
       SET attempts = ?,
           next_attempt_at = datetime('now', '+' || ? || ' seconds'),
           last_error = ?
       WHERE id = ?`).run(attempts, delaySec, error, jobId);
    });
    checkpointAfterWrite();
}
/**
...
export function countPendingJobs() {
    const db = getDb(dbPath());
    const row = db.prepare("SELECT COUNT(*) as count FROM email_suggestion_queue WHERE next_attempt_at <= datetime('now')").get();
    return row.count;
}
