 ...
 */
function resolveCommandFile(projectId, filePath) {
 ...
 *           closes this vector.
 */
function validateCommandPath(filePath, projectId) {
...
export function ensureCommandDir(projectId) {
    const dir = getCommandsBase(projectId);
    if (!existsSync(dir))
        mkdirSync(dir, { recursive: true });
}
/** List all commands registered for a project. */
export function listCommands(projectId) {
    const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
    return db.prepare("SELECT * FROM commands WHERE project_id = ?").all(projectId);
}
/** Get a single command by name. Returns undefined if not found. */
export function getCommand(projectId, name) {
    const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
 ...
 */
export function createCommand(projectId, name, filePath, content) {
    return execTransaction(() => {
        ...
        const body = content ?? "";
        db.prepare(`INSERT INTO commands (id, project_id, name, file_path, content, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`).run(id, projectId, name, filePath, body, now, now);
 ...
 */
export function deleteCommand(projectId, name) {
    return execTransaction(() => {
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
 ...
 */
export function updateCommand(projectId, name, updates) {
    return execTransaction(() => {
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
        ...
        if (updates.file_path)
            validateCommandPath(updates.file_path, projectId);
        db.prepare("UPDATE commands SET file_path = ?, content = ?, updated_at = ? WHERE id = ?").run(newFilePath, newContent, now, existing.id);
        ...
        checkpointAfterWrite();
        return db.prepare("SELECT * FROM commands WHERE id = ?").get(existing.id);
    });
}
