 ...
 */
export function getConfig(projectId, type) {
    const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
    return db.prepare("SELECT * FROM configs WHERE project_id = ? AND type = ?").get(projectId, type);
}
/**
 ...
 */
export function saveConfig(projectId, type, content) {
    const config = execTransaction(() => {
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
        const now = new Date().toISOString();
        const existing = db.prepare("SELECT * FROM configs WHERE project_id = ? AND type = ?").get(projectId, type);
        if (existing) {
 ...
 */
export function syncConfigFromDisk(projectId, type) {
    try {
        const configPath = type === "global"
            ? resolve(resolveProjectBase(projectId), "opencode.jsonc")
            : getConfigPath(projectId);
