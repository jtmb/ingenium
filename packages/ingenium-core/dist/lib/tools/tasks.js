 ...
 */
function logTaskActivity(_projectId, taskId, actor, eventType, payload) {
    // Activity logging runs in its own transaction so it doesn't nest
    // inside the parent transaction of the mutation.
    const id = randomUUID();
    const now = new Date().toISOString();
    const db = getDb(dbPath());
    db.prepare(`INSERT INTO task_activity (id, task_id, actor, event_type, payload, created_at)
     VALUES (?, ?, ?, ?, ?, ?)`).run(id, taskId, actor, eventType, payload ? JSON.stringify(payload) : null, now);
}
// ============================================================================
// Task CRUD
// ============================================================================
export function createTask(projectId, title, description, assignedTo, fields) {
    const result = execTransaction(() => {
        const db = getDb(dbPath());
        const now = new Date().toISOString();
        const id = randomUUID();
        db.prepare(`INSERT INTO tasks (id, project_id, title, description, column_id, assigned_to,
       ...
       VALUES (?, ?, ?, ?, 'todo', ?,
         ?, ?, ?, ?, ?,
         ?, 0, ?, ?,
         ?, ?)`).run(id, projectId, title, description ?? null, assignedTo ?? null, fields?.parent_id ?? null, fields?.issue_type ?? "task", fields?.priority ?? 0, fields?.due_date ?? null, fields?.start_date ?? null, fields?.estimate_minutes ?? null, fields?.estimate_minutes ?? null, fields?.custom_fields ?? null, now, now);
...
export function listTasks(projectId, columnId) {
    const db = getDb(dbPath());
    if (columnId) {
        return db.prepare("SELECT * FROM tasks WHERE project_id = ? AND column_id = ? ORDER BY priority DESC, created_at").all(projectId, columnId);
    }
    return db.prepare("SELECT * FROM tasks WHERE project_id = ? ORDER BY priority DESC, created_at").all(projectId);
}
export function moveTask(taskId, columnId, actor) {
    const result = execTransaction(() => {
        const db = getDb(dbPath());
        const now = new Date().toISOString();
        const completedAt = columnId === "done" ? now : null;
        const prev = db.prepare("SELECT column_id FROM tasks WHERE id = ?").get(taskId);
        db.prepare("UPDATE tasks SET column_id = ?, updated_at = ?, completed_at = ? WHERE id = ?")
            .run(columnId, now, completedAt, taskId);
        return { task: db.prepare("SELECT * FROM tasks WHERE id = ?").get(taskId), prevColumn: prev?.column_id };
    });
    checkpointAfterWrite();
    if (result.task) {
...
export function completeTask(taskId, actor) {
...
export function getNextTask(projectId) {
    ...
    return db.prepare(`SELECT * FROM tasks WHERE project_id = ? AND column_id = 'todo'
     ORDER BY priority DESC, created_at ASC LIMIT 1`).get(projectId);
}
 ...
 */
export function updateTask(projectId, taskId, fields, actor) {
    const result = execTransaction(() => {
        const db = getDb(dbPath());
        const now = new Date().toISOString();
 ...
 */
export function deleteTask(projectId, taskId, actor) {
    const result = execTransaction(() => {
        const db = getDb(dbPath());
        // Check task exists
 ...
 */
export function searchTasks(projectId, query, limit = 50) {
    const db = getDb(dbPath());
    // Sanitize FTS5 query: wrap terms in quotes to avoid syntax errors
    const sanitized = query.replace(/"/g, '""');
    return db.prepare(`SELECT t.*, rank FROM tasks t
 ...
 */
export function getTaskTree(projectId, parentId) {
    const db = getDb(dbPath());
    if (parentId) {
        // Get immediate children of a specific parent
        const children = db.prepare("SELECT * FROM tasks WHERE project_id = ? AND parent_id = ? ORDER BY priority DESC, created_at").all(projectId, parentId);
    ...
    const epics = db.prepare("SELECT * FROM tasks WHERE project_id = ? AND parent_id IS NULL AND issue_type = 'epic' ORDER BY priority DESC, created_at").all(projectId);
...
// Comments
// ============================================================================
export function addComment(projectId, taskId, author, body, parentCommentId, actor) {
    const result = execTransaction(() => {
        const db = getDb(dbPath());
        const now = new Date().toISOString();
        const id = randomUUID();
        db.prepare(`INSERT INTO task_comments (id, task_id, parent_comment_id, author, body, created_at)
       VALUES (?, ?, ?, ?, ?, ?)`).run(id, taskId, parentCommentId ?? null, author, body, now);
...
export function editComment(projectId, commentId, body, actor) {
    const result = execTransaction(() => {
        const db = getDb(dbPath());
        const now = new Date().toISOString();
        const info = db.prepare("UPDATE task_comments SET body = ?, edited_at = ? WHERE id = ?").run(body, now, commentId);
        if (info.changes === 0)
            return undefined;
...
export function reactComment(projectId, commentId, reaction, actor) {
    const result = execTransaction(() => {
        const db = getDb(dbPath());
        // Read existing reactions
        const comment = db.prepare("SELECT reactions, task_id FROM task_comments WHERE id = ?").get(commentId);
        if (!comment)
            return undefined;
        let reactions = {};
        try {
...
export function getComments(_projectId, taskId) {
    const db = getDb(dbPath());
    return db.prepare("SELECT * FROM task_comments WHERE task_id = ? ORDER BY created_at").all(taskId);
}
// ============================================================================
// Activity
// ============================================================================
...
// Links
// ============================================================================
export function linkTasks(projectId, taskId, linkedTaskId, linkType, actor) {
    const result = execTransaction(() => {
        const db = getDb(dbPath());
        const id = randomUUID();
        // Prevent self-links
        ...
        // Check for duplicate
        const existing = db.prepare("SELECT id FROM task_links WHERE task_id = ? AND linked_task_id = ? AND link_type = ?").get(taskId, linkedTaskId, linkType);
        if (existing) {
            return db.prepare("SELECT * FROM task_links WHERE id = ?").get(existing.id);
        }
        db.prepare("INSERT INTO task_links (id, task_id, linked_task_id, link_type) VALUES (?, ?, ?, ?)").run(id, taskId, linkedTaskId, linkType);
...
export function unlinkTasks(projectId, linkId, actor) {
    const result = execTransaction(() => {
        const db = getDb(dbPath());
        // Read link to know which tasks to log
        const link = db.prepare("SELECT task_id, linked_task_id, link_type FROM task_links WHERE id = ?").get(linkId);
...
export function getTaskLinks(_projectId, taskId) {
    const db = getDb(dbPath());
    return db.prepare("SELECT * FROM task_links WHERE task_id = ? OR linked_task_id = ?").all(taskId, taskId);
}
// ============================================================================
// Notifications
// ============================================================================
export function notifyTask(projectId, recipient, taskId, kind) {
    const result = execTransaction(() => {
        const db = getDb(dbPath());
        // Dedupe: skip if same recipient+task+kind with unread already exists
        const existing = db.prepare("SELECT id FROM task_notifications WHERE recipient = ? AND task_id = ? AND kind = ? AND read_at IS NULL").get(recipient, taskId, kind);
        if (existing)
            return null;
        const now = new Date().toISOString();
        const id = randomUUID();
        db.prepare("INSERT INTO task_notifications (id, project_id, recipient, task_id, kind, created_at) VALUES (?, ?, ?, ?, ?, ?)").run(id, projectId, recipient, taskId, kind, now);
        return db.prepare("SELECT * FROM task_notifications WHERE id = ?").get(id);
    });
    checkpointAfterWrite();
    return result;
}
export function getNotifications(projectId, recipient, unreadOnly) {
    const db = getDb(dbPath());
    if (unreadOnly) {
        return db.prepare("SELECT * FROM task_notifications WHERE project_id = ? AND recipient = ? AND read_at IS NULL ORDER BY created_at DESC").all(projectId, recipient);
    }
    return db.prepare("SELECT * FROM task_notifications WHERE project_id = ? AND recipient = ? ORDER BY created_at DESC").all(projectId, recipient);
}
export function markNotificationRead(_projectId, notificationId) {
    const result = execTransaction(() => {
        const db = getDb(dbPath());
        const now = new Date().toISOString();
        const info = db.prepare("UPDATE task_notifications SET read_at = ? WHERE id = ?").run(now, notificationId);
...
export function getBoardConfig(projectId) {
    const result = execTransaction(() => {
        const db = getDb(dbPath());
        // Try to insert default if none exists
        const now = new Date().toISOString();
        db.prepare(`INSERT OR IGNORE INTO board_config (id, project_id, columns, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?)`).run(randomUUID(), projectId, DEFAULT_COLUMNS, now, now);
        return db.prepare("SELECT * FROM board_config WHERE project_id = ?").get(projectId);
    });
    checkpointAfterWrite();
    return result;
}
export function updateBoardConfig(projectId, updates) {
    const result = execTransaction(() => {
        const db = getDb(dbPath());
        const now = new Date().toISOString();
        // Ensure config exists
        const existing = db.prepare("SELECT id FROM board_config WHERE project_id = ?").get(projectId);
        if (!existing) {
            // Create default first, then update
            db.prepare(`INSERT INTO board_config (id, project_id, columns, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?)`).run(randomUUID(), projectId, DEFAULT_COLUMNS, now, now);
        }
        const setClauses = ["updated_at = ?"];
        const params = [now];
 ...
 * not to block moves.
 */
export function validateWipLimit(projectId, columnId) {
    const db = getDb(dbPath());
    // Count tasks in the column
    const countRow = db.prepare("SELECT COUNT(*) as count FROM tasks WHERE project_id = ? AND column_id = ?").get(projectId, columnId);
    // Get board config
...
// Bulk operations
// ============================================================================
export function bulkUpdateTasks(_projectId, taskIds, fields) {
    if (taskIds.length === 0)
        return 0;
    const result = execTransaction(() => {
        const db = getDb(dbPath());
