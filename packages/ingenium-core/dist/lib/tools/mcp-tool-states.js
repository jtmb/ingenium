...
import { MCP_TOOL_CATALOG, getAllToolNames, getToolsByCategory, getCatalogMap, getCategoryOrder, } from "./mcp-tool-catalog.js";
/**
 ...
 */
export function getToolState(projectId, toolName) {
    const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
    const row = db.prepare("SELECT enabled FROM mcp_tool_states WHERE project_id = ? AND tool_name = ?").get(projectId, toolName);
    if (!row)
        return true; // default enabled
    return row.enabled === 1;
}
export function setToolState(projectId, toolName, enabled) {
    ...
    `).run(projectId, toolName, enabled ? 1 : 0, now, now, enabled ? 1 : 0, now);
        return db.prepare("SELECT * FROM mcp_tool_states WHERE project_id = ? AND tool_name = ?").get(projectId, toolName);
    });
 ...
 */
export function listToolStates(projectId) {
    const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
    const rows = db.prepare("SELECT tool_name, enabled FROM mcp_tool_states WHERE project_id = ? ORDER BY tool_name").all(projectId);
    return rows.map(r => ({ tool_name: r.tool_name, enabled: r.enabled === 1 }));
}
/** Derived from the catalog — all known tool names. */
export const ALL_TOOLS = MCP_TOOL_CATALOG.map(e => e.name);
/**
 ...
 */
export function listToolStatesWithDefaults(projectId) {
...
export function listCategorizedTools(projectId) {
 ...
 */
export function setCategoryState(projectId, category, enabled) {
