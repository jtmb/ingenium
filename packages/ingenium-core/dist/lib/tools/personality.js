 ...
 */
export function upsertTrait(projectId, traitType, traitValue, label, confidence, exemplarObservationId, exemplarText) {
    const result = execTransaction(() => {
        ...
        const existing = db.prepare(`SELECT * FROM personality_traits
       WHERE project_id = ? AND trait_type = ? AND trait_value = ?`).get(projectId, traitType, traitValue);
        if (existing) {
            ...
            const params = [newConfidence, now];
            ...
            db.prepare(`UPDATE personality_traits SET ${updates.join(", ")} WHERE id = ?`).run(...params);
            return db.prepare("SELECT * FROM personality_traits WHERE id = ?").get(existing.id);
        }
        // New trait: insert with provided or default confidence
        const insertResult = db.prepare(`INSERT INTO personality_traits (project_id, trait_type, trait_value, display_label, confidence, exemplar_observation_id, exemplar_text, source, is_active, metadata, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, 'synthesis', 1, NULL, ?, ?)`).run(projectId, traitType, traitValue, label ?? null, confidence ?? 0.5, exemplarObservationId ?? null, exemplarText ?? null, now, now);
        return db.prepare("SELECT * FROM personality_traits WHERE id = ?").get(insertResult.lastInsertRowid);
    });
 ...
 */
export function listTraits(projectId, includeInactive = false) {
    const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./.ingenium/data.db");
 ...
 */
export function getTraits(projectId, traitType, minConfidence) {
 ...
 */
export function getProfile(projectId, options) {
    const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./.ingenium/data.db");
    if (options?.includeHidden) {
        // Return all active traits regardless of confidence
        return db.prepare(`SELECT project_id, trait_type,
       ...
       ORDER BY trait_type, confidence DESC`).all(projectId);
    }
 ...
 */
export function disableTrait(id) {
    const ok = execTransaction(() => {
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./.ingenium/data.db");
        const now = new Date().toISOString();
        const result = db.prepare("UPDATE personality_traits SET is_active = 0, updated_at = ? WHERE id = ?").run(now, id);
 ...
 */
export function setActive(projectId, traitId, active) {
    execTransaction(() => {
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./.ingenium/data.db");
 ...
 */
export function updateConfidence(projectId, traitType, traitValue, delta) {
    const result = execTransaction(() => {
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./.ingenium/data.db");
        const existing = db.prepare(`SELECT * FROM personality_traits
       WHERE project_id = ? AND trait_type = ? AND trait_value = ?`).get(projectId, traitType, traitValue);
        ...
        db.prepare("UPDATE personality_traits SET confidence = ?, updated_at = ? WHERE id = ?").run(newConfidence, new Date().toISOString(), existing.id);
        return db.prepare("SELECT * FROM personality_traits WHERE id = ?").get(existing.id);
    });
    if (result) {
        checkpointAfterWrite();
 ...
 */
export function deleteTrait(projectId, id) {
    const ok = execTransaction(() => {
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./.ingenium/data.db");
        const result = db.prepare("DELETE FROM personality_traits WHERE project_id = ? AND id = ?").run(projectId, id);
        return result.changes > 0;
    });
    checkpointAfterWrite();
    return ok;
 ...
 */
export function deleteAllTraits(projectId) {
    const result = execTransaction(() => {
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./.ingenium/data.db");
        const deleteResult = db.prepare("DELETE FROM personality_traits WHERE project_id = ?").run(projectId);
