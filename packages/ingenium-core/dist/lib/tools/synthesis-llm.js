 ...
 */
function buildPrompt(observations, existingSkills, existingTraits) {
    // Format observations
    const obsText = observations.map(o => `  - type:${o.observation_type} importance:${o.importance} "${o.content.substring(0, 200)}"`).join("\n");
...
function validateResponse(raw) {
    ...
    if (Array.isArray(raw.skills_to_create)) {
        result.skills_to_create = raw.skills_to_create.slice(0, 5).map((s) => {
            ...
            if (s.reference_files && Array.isArray(s.reference_files)) {
                item.reference_files = s.reference_files
                    .filter((rf) => rf.path && rf.content && rf.path.startsWith("references/"))
    ...
    if (Array.isArray(raw.skills_to_update)) {
        result.skills_to_update = raw.skills_to_update.slice(0, 5).map((s) => {
            const update = {
                name: String(s.name || ""),
 ...
 */
export async function callSynthesisLLM(observations, existingSkills, existingTraits, endpoint, model, apiKey, signal, allowPrivateNetwork = false) {
 ...
 */
export function resolveLLMConfig(projectId) {
 ...
 */
export function isLLMSynthesisConfigured(projectId) {
 ...
 */
export function getLLMSynthesisConfig(projectId) {
    const config = resolveLLMConfig(projectId);
 ...
 */
export async function consolidateTraits(projectId, observations, existingTraits) {
    const config = resolveLLMConfig(projectId);
    if (!config || !config.model || !config.endpoint)
        return null;
    const { model, apiKey, endpoint } = config;
    // Build prompt
    const obsText = observations.map(o => `  [id:${o.id}] type:${o.observation_type} "${o.content.substring(0, 200)}"`).join("\n");
    const traitsText = existingTraits.length > 0
  ...
  "ignore_count": 1
    ...
    try {
        ...
        if (!parsed)
            return null;
        // Parse defensively
        const create = (Array.isArray(parsed.create) ? parsed.create : []).slice(0, 20).map((c) => ({
            ...
            observation_ids: (Array.isArray(c.observation_ids) ? c.observation_ids : []).map(Number).filter((n) => n > 0),
        })).filter((c) => c.trait_value.length > 0);
        const confirm = (Array.isArray(parsed.confirm) ? parsed.confirm : []).slice(0, 50).map((c) => ({
            trait_id: Number(c.trait_id) || 0,
            observation_id: Number(c.observation_id) || 0,
        })).filter((c) => c.trait_id > 0 && c.observation_id > 0);
 ...
 */
function buildEnrichmentPrompt(observations) {
 ...
 */
function parseEnrichmentResponse(raw, count) {
    const results = [];
    ...
    for (let i = 0; i < count; i++) {
        const entry = raw.find((e) => e?.index === i);
        if (entry) {
            results.push({
 ...
 */
export async function enrichObservations(observations, endpoint, model, apiKey, signal, allowPrivateNetwork = false) {
    if (observations.length === 0)
        return [];
    if (!endpoint || !model) {
 ...
 */
export function getFullLLMSynthesisConfig(projectId) {
    const config = resolveLLMConfig(projectId);
    if (!config || !config.model || !config.endpoint)
        return null;
 ...
 */
export function buildConsolidationPrompt(skills, total) {
    const skillsText = skills.map(s => `- **${s.name}**: ${s.description} [tags: ${s.tags}] — ${s.content_preview}...`).join("\n");
 ...
 */
export async function callConsolidationLLM(projectId, prompt, overrideEndpoint, overrideModel, overrideApiKey) {
    ...
    try {
        ...
        return {
            merges: Array.isArray(parsed.merges)
                ? parsed.merges.filter((m) => m.source && m.target && typeof m.source === "string" && typeof m.target === "string")
                    .map((m) => ({ source: m.source, target: m.target, reason: String(m.reason || "").slice(0, 200) }))
                : [],
            delete: Array.isArray(parsed.delete)
                ? parsed.delete.filter((d) => typeof d === "string" && d.length > 0).map(String)
                : [],
        };
    }
    catch (err) {
