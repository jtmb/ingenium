 ...
 */
export function isValidSkillFileTree(value) {
...
function safeSkillsBaseRoot(skillsBase) {
    ...
    try {
        ...
        if (!canon.startsWith(parentCanon + sep) && canon !== parentCanon)
            return false;
        return true;
    }
    catch {
 ...
 */
function safeSkillDir(skillsBase, name) {
    if (!safeSkillsBaseRoot(skillsBase))
        return null;
    assertSafeName(name);
    ...
    try {
        ...
        if (!canon.startsWith(canonBase + sep) && canon !== canonBase)
            return null;
        return canon;
    }
    catch {
...
function resolveSafePath(baseDir, relativePath) {
    ...
    if (resolved === canonicalSkillMd || resolved === canonicalMetadataJson)
        return null;
        ...
        for (;;) {
            ...
            if (existsSync(walk)) {
                try {
                    ...
                    if (!canon.startsWith(baseDir + sep) && canon !== baseDir)
                        return null;
                }
...
export function listArchivedSkills(projectId) {
    const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
 ...
 */
export function getSkillVersions(skillId) {
    const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
    return db.prepare("SELECT * FROM skill_versions WHERE skill_id = ? ORDER BY revision DESC").all(skillId);
}
/**
 * Get a specific version of a skill by revision number.
 */
export function getSkillVersion(skillId, revision) {
    const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
    return db.prepare("SELECT * FROM skill_versions WHERE skill_id = ? AND revision = ?").get(skillId, revision);
}
/**
        ...
        try {
            ...
            for (const [relPath, content] of Object.entries(tree)) {
                if (typeof content !== "string")
                    continue; // defensively skip non-string values
...
export function removeSkillMdOnly(name, projectId) {
    if (!isSafeSkillName(name)) {
 ...
 */
function removeSkillMdFromDisk(name, projectId) {
...
export function createSkill(projectId, name, description, content, category, tags, alwaysApply, fileTree) {
    ...
    const skill = execTransaction(() => {
        ...
        if (existing) {
         ...
         WHERE project_id = ? AND name = ?`).run(description, content, category ?? null, tags ?? null, alwaysApply ?? 0, fileTree ?? null, newRevision, now, projectId, name);
        }
        else {
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?)`).run(id, projectId, name, description, content, category ?? null, tags ?? null, alwaysApply ?? 0, fileTree ?? null, now, now);
        }
...
export function updateSkill(projectId, name, content, description, tags, alwaysApply, fileTree) {
    ...
    const skill = execTransaction(() => {
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
        const now = new Date().toISOString();
        const current = db.prepare("SELECT revision FROM skills WHERE project_id = ? AND name = ?").get(projectId, name);
        if (!current)
            return undefined;
        ...
        db.prepare(`UPDATE skills SET
         ...
         revision = ?, updated_at = ?
       WHERE project_id = ? AND name = ?`).run(content, description ?? null, tags ?? null, alwaysApply ?? null, fileTree ?? null, newRevision, now, projectId, name);
        return db.prepare("SELECT * FROM skills WHERE project_id = ? AND name = ?")
            .get(projectId, name);
    });
    checkpointAfterWrite();
...
export function enableSkill(projectId, name) {
    ...
    const skill = execTransaction(() => {
        ...
        if (!current)
            return undefined;
        const newRevision = current.revision + 1;
        db.prepare("UPDATE skills SET enabled = 1, revision = ?, updated_at = ? WHERE project_id = ? AND name = ?").run(newRevision, now, projectId, name);
        return db.prepare("SELECT * FROM skills WHERE project_id = ? AND name = ?")
            .get(projectId, name);
    });
    checkpointAfterWrite();
...
export function disableSkill(projectId, name) {
    ...
    const skill = execTransaction(() => {
        ...
        if (!current)
            return undefined;
        const newRevision = current.revision + 1;
        db.prepare("UPDATE skills SET enabled = 0, revision = ?, updated_at = ? WHERE project_id = ? AND name = ?").run(newRevision, now, projectId, name);
        return db.prepare("SELECT * FROM skills WHERE project_id = ? AND name = ?")
            .get(projectId, name);
    });
    checkpointAfterWrite();
    ...
    if (skill)
        removeSkillMdFromDisk(name, projectId);
    return skill;
}
/**
 ...
 * Returns undefined if the skill doesn't exist.
 */
export function archiveSkill(projectId, name) {
    assertSafeName(name);
    const skill = execTransaction(() => {
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
        const current = db.prepare("SELECT revision, archived_at FROM skills WHERE project_id = ? AND name = ?").get(projectId, name);
        if (!current)
            return undefined;
        ...
        const now = new Date().toISOString();
        const newRevision = current.revision + 1;
        db.prepare("UPDATE skills SET archived_at = ?, revision = ?, updated_at = ? WHERE project_id = ? AND name = ?").run(now, newRevision, now, projectId, name);
        return db.prepare("SELECT * FROM skills WHERE project_id = ? AND name = ?")
            .get(projectId, name);
    });
    checkpointAfterWrite();
 ...
 * Returns undefined if the skill doesn't exist.
 */
export function restoreSkill(projectId, name) {
    assertSafeName(name);
    const skill = execTransaction(() => {
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
        const current = db.prepare("SELECT revision, archived_at FROM skills WHERE project_id = ? AND name = ?").get(projectId, name);
        ...
        const now = new Date().toISOString();
        const newRevision = current.revision + 1;
        db.prepare("UPDATE skills SET archived_at = NULL, revision = ?, updated_at = ? WHERE project_id = ? AND name = ?").run(newRevision, now, projectId, name);
        return db.prepare("SELECT * FROM skills WHERE project_id = ? AND name = ?")
            .get(projectId, name);
    });
    checkpointAfterWrite();
 ...
 */
export function rollbackSkill(projectId, name, targetRevision) {
    assertSafeName(name);
    const skill = execTransaction(() => {
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
        const current = db.prepare("SELECT id, revision FROM skills WHERE project_id = ? AND name = ?").get(projectId, name);
        if (!current)
            return undefined;
        // Load the target version
        const version = db.prepare("SELECT * FROM skill_versions WHERE skill_id = ? AND revision = ?").get(current.id, targetRevision);
        if (!version)
            return undefined;
        ...
        db.prepare(`UPDATE skills SET
         ...
         enabled = ?, archived_at = ?,
         revision = ?, updated_at = ?
       WHERE id = ?`).run(version.name, version.description, version.content, version.category, version.tags, version.always_apply, version.file_tree, version.enabled, version.archived_at, newRevision, now, current.id);
...
export function syncSkillFromDisk(projectId, name) {
    ...
    if (!existsSync(filePath)) {
        ...
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
        const existing = db.prepare("SELECT * FROM skills WHERE project_id = ? AND name = ?").get(projectId, name);
    ...
    const result = execTransaction(() => {
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
        const existing = db.prepare("SELECT * FROM skills WHERE project_id = ? AND name = ?").get(projectId, diskName);
        if (!existing) {
            const now = new Date().toISOString();
            const id = randomUUID();
            db.prepare(`INSERT INTO skills (id, project_id, name, description, content, tags, always_apply, file_tree, revision, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?)
         ON CONFLICT(project_id, name) DO UPDATE SET
           ...
           archived_at = NULL, updated_at = excluded.updated_at`).run(id, projectId, diskName, parsedDescription, parsedContent, diskTags, diskAlwaysApply, fileTree, now, now);
            ...
            const existingFileTree = existing.file_tree || "";
        ...
        return db.prepare("SELECT * FROM skills WHERE project_id = ? AND name = ?").get(projectId, diskName);
    });
    checkpointAfterWrite();
    return result;
}
...
export function syncAllSkills(projectId, db) {
    ...
    const skills = database.prepare("SELECT * FROM skills WHERE project_id = ? AND enabled = 1 AND archived_at IS NULL").all(projectId);
    let written = 0;
...
export function copySkills(sourceProjectId, targetProjectId) {
    const count = execTransaction(() => {
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
        const sourceSkills = db.prepare("SELECT * FROM skills WHERE project_id = ? AND enabled = 1 AND archived_at IS NULL").all(sourceProjectId);
        let n = 0;
        for (const skill of sourceSkills) {
            ...
            const id = randomUUID();
            db.prepare(`INSERT INTO skills (id, project_id, name, description, content, category, tags, always_apply, file_tree, revision, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?)`).run(id, targetProjectId, skill.name, skill.description, skill.content, skill.category ?? null, skill.tags ?? null, skill.always_apply ?? 0, skill.file_tree ?? null, now, now);
            n++;
        }
        return n;
    });
