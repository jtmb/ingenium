 ...
 */
export function logEvent(projectId, eventType, eventSource, title, description, data, parentEventId, sessionId, importance) {
    const event = execTransaction(() => {
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./.ingenium/data.db");
        const now = new Date().toISOString();
        const result = db.prepare(`INSERT INTO pipeline_events (project_id, event_type, event_source, title, description, data, parent_event_id, session_id, importance, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(projectId, eventType, eventSource, title, description ?? null, data ? JSON.stringify(data) : null, parentEventId ?? null, sessionId ?? null, importance ?? 5, now);
        return db.prepare("SELECT * FROM pipeline_events WHERE id = ?").get(result.lastInsertRowid);
    });
 ...
 */
export function getEvents(projectId, options) {
    const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./.ingenium/data.db");
    ...
    params.push(limit);
    return db.prepare(`SELECT * FROM pipeline_events WHERE ${clauses.join(" AND ")} ORDER BY created_at DESC LIMIT ?`).all(...params);
}
/**
 ...
 */
export function getTimeline(projectId, options) {
