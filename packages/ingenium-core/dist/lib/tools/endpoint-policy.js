...
export async function validateEndpointUrl(endpoint, allowPrivate) {
    let parsed;
    try {
    ...
    try {
        ...
        if (err instanceof Error && err.message.includes("internal/private"))
            throw err;
    }
}
export async function safeLlmFetch(url, init, policy) {
...
async function fetchFollowingAllowedRedirects(url, init, policy, redirects) {
