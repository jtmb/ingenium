...
import { decryptSecret, deriveKey, encryptSecret, generateDEK, generateSalt, unwrapKey, verifyHMAC, wrapKey, } from "./vault-crypto.js";
...
function insertAudit(projectId, eventType, itemId, actor, details) {
    const db = getDb(dbPath());
    db.prepare(`INSERT INTO vault_audit_log (project_id, event_type, item_id, actor, details, created_at)
     VALUES (?, ?, ?, ?, ?, ?)`).run(projectId, eventType, itemId, actor, JSON.stringify(details), new Date().toISOString());
}
...
export function initVault(_projectId, passphrase) {
    ...
    execTransaction(() => {
        const db = getDb(dbPath());
        db.prepare(`INSERT OR IGNORE INTO vault_config
       (id, sealed, master_key_salt, master_key_verify, scrypt_N, scrypt_r, scrypt_p)
       VALUES (1, 1, ?, ?, 16384, 8, 1)`).run(salt, verifyTag);
    });
    checkpointAfterWrite();
}
/** Initialize and unseal a new vault after validating the requested passphrase. */
export function initializeVault(projectId, passphrase, confirmation) {
    if (vaultConfigExists())
        return { ok: false, error: "Vault is already initialized" };
...
export function unsealVault(projectId, passphrase) {
    const db = getDb(dbPath());
    const config = db.prepare("SELECT master_key_salt, master_key_verify, scrypt_N, scrypt_r, scrypt_p FROM vault_config WHERE id = 1").get();
    if (!config)
...
export function createItem(projectId, name, type, value, folderId, tags, urls, username) {
    if (isSealed())
    ...
    execTransaction(() => {
        ...
        db.prepare(`INSERT INTO vault_items
       (id, project_id, folder_id, name, type, tags, urls, username, encrypted, wrapped_kek, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, projectId, folderId ?? null, name, type, JSON.stringify(tags ?? []), JSON.stringify(urls ?? []), username ?? null, encrypted, wrappedDek, now, now);
...
export function getItemMetadata(projectId, itemId) {
    if (isSealed())
        return null;
    const row = getDb(dbPath()).prepare(`SELECT id, name, type, folder_id, tags, urls, username, version, created_at, updated_at, last_accessed_at, access_count
     FROM vault_items WHERE project_id = ? AND id = ? AND access_policy <> ?`).get(projectId, itemId, DELETED_POLICY);
    return row ? toMetadata(row) : null;
}
/** Decrypt a vault item and update its access metadata. */
export function decryptItem(projectId, itemId) {
    let key;
    try {
        key = getMasterKey();
    }
    ...
    const result = execTransaction(() => {
        const db = getDb(dbPath());
        const item = db.prepare("SELECT encrypted, wrapped_kek FROM vault_items WHERE project_id = ? AND id = ? AND access_policy <> ?").get(projectId, itemId, DELETED_POLICY);
        if (!item)
            return null;
        ...
        try {
            ...
            const now = new Date().toISOString();
            db.prepare("UPDATE vault_items SET last_accessed_at = ?, access_count = access_count + 1 WHERE project_id = ? AND id = ?").run(now, projectId, itemId);
...
export function listItems(projectId, folderId) {
    if (isSealed())
        return [];
    const db = getDb(dbPath());
    const rows = folderId === undefined
        ? db.prepare(`SELECT id, name, type, folder_id, tags, urls, username, version, created_at, updated_at, last_accessed_at, access_count
       FROM vault_items WHERE project_id = ? AND access_policy <> ? ORDER BY name`).all(projectId, DELETED_POLICY)
        : db.prepare(`SELECT id, name, type, folder_id, tags, urls, username, version, created_at, updated_at, last_accessed_at, access_count
       FROM vault_items WHERE project_id = ? AND folder_id = ? AND access_policy <> ? ORDER BY name`).all(projectId, folderId, DELETED_POLICY);
    return rows.map(toMetadata);
}
/** Re-encrypt an active vault item under a fresh data encryption key. */
export function updateItem(projectId, itemId, value) {
    if (isSealed())
        return;
    const dek = generateDEK();
    const key = getMasterKey();
    ...
    execTransaction(() => {
        const db = getDb(dbPath());
        const changed = db.prepare(`UPDATE vault_items
       SET encrypted = ?, wrapped_kek = ?, version = version + 1, updated_at = ?
       WHERE project_id = ? AND id = ? AND access_policy <> ?`).run(encrypted, wrappedDek, new Date().toISOString(), projectId, itemId, DELETED_POLICY);
        if (changed.changes > 0)
            insertAudit(projectId, "secret_updated", itemId, "system", {});
    });
    checkpointAfterWrite();
}
/** Update non-sensitive metadata for an active vault item. */
export function updateItemMetadata(projectId, itemId, updates) {
    if (isSealed())
        return false;
    const fields = [];
    const values = [];
    ...
    if (updates.type !== undefined)
        add("type", updates.type);
    if (updates.folderId !== undefined)
        add("folder_id", updates.folderId);
    if (updates.tags !== undefined)
        add("tags", JSON.stringify(updates.tags));
    if (updates.urls !== undefined)
        add("urls", JSON.stringify(updates.urls));
    if (updates.username !== undefined)
        add("username", updates.username);
    if (fields.length === 0)
        return getItemMetadata(projectId, itemId) !== null;
    let changed = false;
    execTransaction(() => {
        const result = getDb(dbPath()).prepare(`UPDATE vault_items
       SET ${fields.join(", ")}, updated_at = ?
       WHERE project_id = ? AND id = ? AND access_policy <> ?`).run(...values, new Date().toISOString(), projectId, itemId, DELETED_POLICY);
...
export function deleteItem(projectId, itemId) {
    ...
    execTransaction(() => {
        const db = getDb(dbPath());
        const changed = db.prepare("UPDATE vault_items SET access_policy = ?, updated_at = ? WHERE project_id = ? AND id = ? AND access_policy <> ?").run(DELETED_POLICY, new Date().toISOString(), projectId, itemId, DELETED_POLICY);
        if (changed.changes > 0)
            insertAudit(projectId, "secret_deleted", itemId, "system", {});
    });
    checkpointAfterWrite();
}
/** Persist an auditable vault event. */
export function logAudit(projectId, eventType, itemId, actor, details) {
