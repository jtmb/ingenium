 ...
 */
function writeAgentToDisk(agent) {
 ...
 */
export function createAgent(projectId, name, content, description, category, mode, model, enabled = true) {
    ...
    const agent = execTransaction(() => {
        ...
        const now = new Date().toISOString();
        const id = randomUUID();
        db.prepare(`INSERT INTO agents (id, project_id, name, description, category, mode, model, content, enabled, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, projectId, name, description ?? "", safeCategory, mode ?? "subagent", model ?? null, content, enabled ? 1 : 0, now, now);
...
export function updateAgent(projectId, name, updates) {
    ...
    const agent = execTransaction(() => {
        ...
        db.prepare(`UPDATE agents SET description = ?, category = ?, mode = ?, model = ?, content = ?, updated_at = ? WHERE id = ?`).run(newDesc, newCat, newMode, newModel, newContent, now, existing.id);
...
export function syncAgentFromDisk(projectId, name) {
    ...
    const agent = execTransaction(() => {
        const now = new Date().toISOString();
        if (dbAgent) {
            db.prepare(`UPDATE agents SET name = ?, description = ?, category = ?, mode = ?, model = ?, reasoning_effort = ?, permissions = ?, skills = ?, content = ?, updated_at = ? WHERE id = ?`).run(agentName, description, category, mode, model, reasoningEffort, permissions, JSON.stringify(skillMatches), body, now, dbAgent.id);
        }
        else {
            const id = randomUUID();
            db.prepare(`INSERT OR IGNORE INTO agents (id, project_id, name, description, category, mode, model, reasoning_effort, permissions, skills, content, enabled, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?)`).run(id, projectId, agentName, description, category, mode, model, reasoningEffort, permissions, JSON.stringify(skillMatches), body, now, now);
        }
        return db.prepare("SELECT * FROM agents WHERE project_id = ? AND name = ?")
