...
export const AGENT_CATEGORIES = ["primary", "execution", "research", "security", "chat"];
...
export function isSerializedAgentObject(value) {
...
export function isReservedAgentName(value) {
    return value === LLM_BROKER_AGENT;
}
function parseSerializedAgentObject(value) {
    if (!value)
        return {};
    try {
...
function serializeAgentObject(value) {
    return JSON.stringify(parseSerializedAgentObject(value));
}
function hasOnlyExpectedEntries(value, expected) {
...
export function isCanonicalBrokerPermissions(value) {
    if (!isSerializedAgentObject(value))
        return false;
...
function canonicalMetadataForDisk(name, metadata) {
...
function parseYamlMappingKey(value) {
 ...
 */
function parsePermissionFrontmatter(frontmatter) {
...
function reservedBrokerFileContent() {
...
export function isCanonicalBrokerAgent(agent) {
...
export function isAgentCategory(category) {
    return typeof category === "string" && AGENT_CATEGORIES.includes(category);
}
function assertSafeAgentName(name) {
    if (!isSafeAgentName(name))
        throw new Error("Invalid agent name");
}
function assertAgentCategory(category) {
...
function readProjectConfig(projectId) {
    ...
    const stored = db.prepare("SELECT content FROM configs WHERE project_id = ? AND type = 'project'").get(projectId)?.content;
    if (stored) {
        try {
...
function configuredAgentModel(projectId, name) {
    const entry = readProjectConfig(projectId).agent;
    return typeof entry?.[name]?.model === "string" ? entry[name].model : null;
}
function updateAgentRuntimeConfig(projectId, name, options) {
    ...
    const content = execTransaction(() => {
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
        const stored = db.prepare("SELECT * FROM configs WHERE project_id = ? AND type = 'project'").get(projectId);
        let config;
        try {
 ...
 */
function writeAgentToDisk(agent, force = false) {
    assertSafeAgentName(agent.name);
    assertAgentCategory(agent.category);
    if (!agent.enabled && !force)
        return;
    ...
    if (existsSync(filePath)) {
        ...
        if (fmMatch) {
            ...
            const metadata = canonicalMetadataForDisk(agent.name, parseSerializedAgentObject(agent.metadata));
    ...
    const permissions = defaultPermissionsForAgent(agent.name, parseSerializedAgentObject(agent.permissions));
    const metadata = canonicalMetadataForDisk(agent.name, parseSerializedAgentObject(agent.metadata));
    const skills = (() => { try {
    ...
    if (agent.reasoning_effort)
        frontmatter.push(`reasoning_effort: "${agent.reasoning_effort}"`);
    if (metadata.hidden === true)
 ...
 */
function removeAgentFromDisk(agent) {
    assertSafeAgentName(agent.name);
    assertAgentCategory(agent.category);
 ...
 */
function restoreReservedBrokerFromTrustedState(agent) {
...
function quarantineUntrustedBrokerFiles() {
    ...
    for (const category of AGENT_CATEGORIES) {
        ...
        try {
            ...
            if (!filePath.startsWith(categoryDir + sep) || !existsSync(filePath))
                continue;
            unlinkSync(filePath);
 ...
 */
export function listAgents(projectId, category) {
    const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
    if (category) {
        if (!isAgentCategory(category))
            return [];
...
export function getAgent(projectId, name) {
    if (!isSafeAgentName(name))
        return undefined;
    const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
 ...
 */
export function createAgent(projectId, name, content, description, category, mode, model, enabled = true, permissions, metadata) {
    ...
    const agent = execTransaction(() => {
        ...
        const now = new Date().toISOString();
        const id = randomUUID();
        db.prepare(`INSERT INTO agents (id, project_id, name, description, category, mode, model, permissions, metadata, content, enabled, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, projectId, name, description ?? "", safeCategory, mode ?? "subagent", model ?? null, canonicalPermissionsForAgent(name, permissions), canonicalMetadataForAgent(name, metadata), content, enabled ? 1 : 0, now, now);
...
export function bootstrapReservedBroker(projectId) {
    const broker = execTransaction(() => {
        ...
        const now = new Date().toISOString();
        const id = randomUUID();
        db.prepare(`INSERT INTO agents (id, project_id, name, description, category, mode, model, reasoning_effort, permissions, metadata, skills, content, enabled, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, NULL, NULL, ?, ?, ?, ?, 1, ?, ?)`).run(id, projectId, LLM_BROKER_AGENT, LLM_BROKER_DESCRIPTION, LLM_BROKER_CATEGORY, LLM_BROKER_MODE, LLM_BROKER_PERMISSIONS, LLM_BROKER_METADATA, LLM_BROKER_SKILLS, LLM_BROKER_CONTENT, now, now);
 ...
 */
export function updateAgent(projectId, name, updates) {
    if (!isSafeAgentName(name))
        return undefined;
    ...
    if (isReservedAgentName(name))
        return undefined;
    if (updates.category !== undefined && !isAgentCategory(updates.category))
        return undefined;
    const updated = execTransaction(() => {
        ...
        const newPermissions = canonicalPermissionsForAgent(name, updates.permissions !== undefined ? updates.permissions : existing.permissions);
        const newMetadata = canonicalMetadataForAgent(name, updates.metadata !== undefined ? updates.metadata : existing.metadata);
        db.prepare(`UPDATE agents SET description = ?, category = ?, mode = ?, model = ?, permissions = ?, metadata = ?, content = ?, updated_at = ? WHERE id = ?`).run(newDesc, newCat, newMode, newModel, newPermissions, newMetadata, newContent, now, existing.id);
...
export function deleteAgent(projectId, name) {
    if (!isSafeAgentName(name))
        return false;
    if (isReservedAgentName(name))
        return false;
...
export function enableAgent(projectId, name) {
    if (!isSafeAgentName(name))
        return undefined;
...
export function disableAgent(projectId, name) {
    if (!isSafeAgentName(name))
        return undefined;
 ...
 * - Skills list
 */
export function syncAgentFromDisk(projectId, name) {
    if (!isSafeAgentName(name))
        return undefined;
    ...
    if (isReservedAgentName(name)) {
        if (dbAgent)
            return restoreReservedBrokerFromTrustedState(dbAgent);
    ...
    if (dbAgent) {
        if (!isAgentCategory(dbAgent.category))
            return undefined;
        filePath = resolve(process.env.INGENIUM_CORE_DB_PATH ?? "./data", "..", "..", ".opencode", "agents", dbAgent.category, `${name}.md`);
        category = dbAgent.category;
    }
    else {
        for (const cat of categories) {
            const candidate = resolve(process.env.INGENIUM_CORE_DB_PATH ?? "./data", "..", "..", ".opencode", "agents", cat, `${name}.md`);
    ...
    if (!isSafeAgentName(agentName) || agentName !== name || !isAgentCategory(category))
        return undefined;
    ...
    const permissions = canonicalPermissionsForAgent(agentName, JSON.stringify(defaultPermissionsForAgent(agentName, parsePermissionFrontmatter(frontmatter))));
    const metadata = canonicalMetadataForAgent(agentName, JSON.stringify(parseAgentMetadata(frontmatter)));
    const agent = execTransaction(() => {
        const now = new Date().toISOString();
        if (dbAgent) {
            db.prepare(`UPDATE agents SET name = ?, description = ?, category = ?, mode = ?, model = ?, reasoning_effort = ?, permissions = ?, metadata = ?, skills = ?, content = ?, updated_at = ? WHERE id = ?`).run(agentName, description, category, mode, model, reasoningEffort, permissions, metadata, JSON.stringify(skillMatches), body, now, dbAgent.id);
        }
        else {
            const id = randomUUID();
            db.prepare(`INSERT OR IGNORE INTO agents (id, project_id, name, description, category, mode, model, reasoning_effort, permissions, metadata, skills, content, enabled, created_at, updated_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?)`).run(id, projectId, agentName, description, category, mode, model, reasoningEffort, permissions, metadata, JSON.stringify(skillMatches), body, now, now);
        }
        return db.prepare("SELECT * FROM agents WHERE project_id = ? AND name = ?")
            .get(projectId, agentName);
    });
    if (agent && !dbAgent)
        updateAgentRuntimeConfig(projectId, name, { model: agent.model ?? undefined, disabled: true });
    if (agent && isReservedAgentName(agent.name))
        writeAgentToDisk(agent, true);
    checkpointAfterWrite();
    return agent;
}
