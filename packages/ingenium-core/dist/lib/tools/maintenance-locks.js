 ...
 */
function validateLockInputs(resource, projectId, ownerToken) {
 ...
 */
export function acquireLock(resource, projectId, ownerToken, ttlMs = 30_000) {
    ...
    const { acquired, hadWrites } = execTransaction(() => {
        ...
        const pruneResult = db.prepare("DELETE FROM maintenance_locks WHERE expires_at < ?").run(now);
        ...
        const conflict = projectId === "*"
            ? db.prepare("SELECT 1 FROM maintenance_locks WHERE resource = ? LIMIT 1").get(resource)
            : db.prepare("SELECT 1 FROM maintenance_locks WHERE resource = ? AND (project_id = '*' OR project_id = ?) LIMIT 1").get(resource, projectId);
        if (conflict)
            return { acquired: false, hadWrites: writes > 0 };
        ...
        const insertResult = db.prepare(`INSERT INTO maintenance_locks (resource, project_id, owner_token, acquired_at, expires_at)
       VALUES (?, ?, ?, ?, ?)
       ON CONFLICT(resource, project_id) DO NOTHING`).run(resource, projectId, ownerToken, now, expires);
 ...
 */
export function releaseLock(resource, projectId, ownerToken) {
    validateLockInputs(resource, projectId, ownerToken);
    const { released, hadWrites } = execTransaction(() => {
        const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
        const r = db.prepare("DELETE FROM maintenance_locks WHERE resource = ? AND project_id = ? AND owner_token = ?").run(resource, projectId, ownerToken);
 ...
 */
export function getLockStatus(resource, projectId = "*") {
    const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
    const now = new Date().toISOString();
    const lock = db.prepare(`SELECT * FROM maintenance_locks
     WHERE resource = ? AND (project_id = ? OR project_id = '*')
       AND expires_at > ?
     ORDER BY project_id = '*' ASC
     LIMIT 1`).get(resource, projectId, now);
    return lock;
}
/**
 ...
 */
export function listActiveLocks(resourcePrefix) {
    const db = getDb(process.env.INGENIUM_CORE_DB_PATH ?? "./data");
    const now = new Date().toISOString();
    if (resourcePrefix) {
        return db.prepare("SELECT * FROM maintenance_locks WHERE resource LIKE ? AND expires_at > ? ORDER BY resource, project_id").all(resourcePrefix + "%", now);
    }
    return db.prepare("SELECT * FROM maintenance_locks WHERE expires_at > ? ORDER BY resource, project_id").all(now);
}
/**
 ...
 */
export function renewLock(resource, projectId, ownerToken, ttlMs = 30_000) {
    validateLockInputs(resource, projectId, ownerToken);
    validateTtl(ttlMs);
    const { renewed, hadWrites } = execTransaction(() => {
        ...
        const r = db.prepare(`UPDATE maintenance_locks
       SET expires_at = ?
       WHERE resource = ? AND project_id = ? AND owner_token = ? AND expires_at > ?`).run(newExpires, resource, projectId, ownerToken, now);
