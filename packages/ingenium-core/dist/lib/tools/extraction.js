 ...
 */
function isReasonableLength(text) {
...
function getSeenHashes(projectId) {
    try {
...
function saveSeenHashes(projectId, hashes) {
 ...
 */
function getWatermark(projectId) {
...
function setWatermark(projectId, ts) {
 ...
 */
async function fetchMessages(watermark, limit, projectName) {
 ...
 */
function buildBatchUserPrompt(messages) {
...
export async function callLLMForExtraction(messages, config) {
...
export function parseExtractionResponse(raw) {
    ...
    for (const r of rules) {
        if (!r || typeof r.content !== "string" || r.content.length < 20)
            continue;
        ...
        if (content.length < 20 || !content.startsWith("User "))
            continue;
        ...
        const importance = typeof r.importance === "number"
            ...
            : 6;
        result.push({ content, type: mappedType, importance });
    }
    return result;
}
// ── Main export ──────────────────────────────────────────
export async function runExtraction(projectId, projectName, opts) {
    ...
    try {
        ...
        for (let i = 0; i < rawCandidates.length; i += BATCH_SIZE) {
            ...
            for (const rule of rules) {
                ...
                const obsType = rule.type;
                // Find the originating message for context
                const originatingMsg = batch.find(m => m.text.toLowerCase().includes(rule.content.toLowerCase().slice(0, 30)));
                try {
                    ...
                    storeObservation(projectId, obsType, rule.content, rule.importance ?? 6, "auto-observer", context, originatingMsg?.sessionId);
                    created++;
                }
                catch (err) {
        ...
        try {
            logEvent(projectId, "extraction_completed", "system", `Extraction completed: ${created} observation(s) created`, `Scanned ${scanned} messages, ${candidates} candidates, created ${created} observations, skipped ${skipped}`, {
                scanned,
                candidates,
                created,
                skipped,
        ...
        try {
            logEvent(projectId, "extraction_failed", "system", `Extraction failed: ${String(err?.message || err)}`, undefined, { scanned, candidates, created, skipped, failedBatches: 0, error: String(err?.message || err) });
        }
        catch {
            // Non-fatal
        }
