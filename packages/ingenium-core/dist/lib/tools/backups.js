...
function backupDirectory(dbPath) {
    return process.env.INGENIUM_BACKUPS_DIR ?? resolve(dirname(dbPath), "backups");
}
function sha256(filePath) {
    return createHash("sha256").update(readFileSync(filePath)).digest("hex");
}
function parseManifest(components) {
    try {
        const manifest = JSON.parse(components);
...
function componentPath(dbPath, filename) {
...
export async function createSnapshot(projectId, backupType, dbPath, opencodeDbPath) {
    if (!BACKUP_TYPES.has(backupType))
        throw new Error(`Unsupported backup type: ${backupType}`);
    if (!existsSync(opencodeDbPath))
        throw new Error(`OpenCode database does not exist: ${opencodeDbPath}`);
    ...
    try {
        ...
        execTransaction(() => {
            getDb(dbPath).prepare(`INSERT INTO backup_records
         (id, project_id, filename, size_bytes, sha256, backup_type, components, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, 'completed')`).run(backupId, projectId, filename, sizeBytes, combinedSha256, backupType, manifestJson);
        });
...
export function listBackups(projectId) {
    return getDb(coreDbPath()).prepare("SELECT * FROM backup_records WHERE project_id = ? ORDER BY created_at DESC").all(projectId);
}
/** Get one backup record scoped to its owning project. */
export function getBackup(projectId, backupId) {
    return getDb(coreDbPath()).prepare("SELECT * FROM backup_records WHERE project_id = ? AND id = ?").get(projectId, backupId) ?? null;
}
/** Resolve a validated snapshot component path for streaming or restore. */
export function getBackupComponentPath(projectId, backupId, component = "ingenium") {
...
export function deleteBackup(projectId, backupId) {
...
export function validateRestorePreflight(backupId) {
    const dbPath = coreDbPath();
    const backup = getDb(dbPath).prepare("SELECT * FROM backup_records WHERE id = ?").get(backupId);
    const errors = [];
    const manifest = backup ? parseManifest(backup.components) : null;
    if (!backup)
        return { valid: false, errors: ["Backup record not found"], manifest: {} };
    if (!manifest)
        return { valid: false, errors: ["Backup manifest is invalid"], manifest: {} };
    ...
    const calculatedCombinedSha = createHash("sha256")
        ...
        .digest("hex");
    if (backup.sha256 !== calculatedCombinedSha)
        errors.push("Backup manifest checksum mismatch");
    ...
    if (ingeniumPath && existsSync(ingeniumPath)) {
        ...
        try {
            ...
            if (integrity !== "ok")
                errors.push("Ingenium snapshot integrity check failed");
            const migration = snapshotDb.prepare("SELECT count(*) AS count FROM sqlite_master WHERE type = 'table' AND name = 'backup_records'").get();
            if (migration.count === 0)
                errors.push("Ingenium snapshot is incompatible with migration 047");
        }
        finally {
            snapshotDb.close();
        }
...
export function startRestore(projectId, backupId) {
    const backup = getBackup(projectId, backupId);
    if (!backup)
        throw new Error("Backup not found for project");
    const jobId = randomUUID();
    const now = new Date().toISOString();
    execTransaction(() => {
        getDb(coreDbPath()).prepare(`INSERT INTO backup_restore_jobs (id, project_id, backup_id, status, components, started_at)
       VALUES (?, ?, ?, 'validating', ?, ?)`).run(jobId, projectId, backupId, backup.components, now);
    });
    checkpointAfterWrite();
    return jobId;
}
/** Update a restore job state and record terminal completion timestamps. */
export function updateRestoreStatus(jobId, status, error) {
    if (!RESTORE_STATUSES.has(status))
        throw new Error(`Unsupported restore status: ${status}`);
    const now = new Date().toISOString();
    execTransaction(() => {
        ...
        db.prepare(`UPDATE backup_restore_jobs
       ...
       WHERE id = ?`).run(status, error ?? null, status, now, status, now, jobId);
    });
    checkpointAfterWrite();
}
/** Get the current state of a restore job. */
export function getRestoreStatus(jobId) {
    return getDb(coreDbPath()).prepare("SELECT * FROM backup_restore_jobs WHERE id = ?").get(jobId) ?? null;
}
