...
export function resolveCoreDbPath(requestedPath) {
    ...
    if (existsSync(DEPLOYED_CORE_DB_PATH) ||
 ...
 */
function enforceReservedBrokerInvariant(db) {
    const agentsTable = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type = 'table' AND name = 'agents'").get();
    if (agentsTable.count === 0)
        return;
    const metadataColumn = db.prepare("SELECT count(*) as count FROM pragma_table_info('agents') WHERE name = 'metadata'").get();
    if (metadataColumn.count === 0)
        return;
...
function runMigrations(db) {
    ...
    if (tableCount.count === 0) {
        ...
        const pipelineCreateSql = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='pipeline_events'").get();
        ...
        const jobsCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='jobs'").get();
        ...
        const emailCacheCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='email_cache'").get();
        ...
        const serversCreateSql = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='servers'").get();
        ...
        const skillsCreateSql = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='skills'").get();
        ...
        const emailSuggestionsCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='email_suggestions'").get();
        ...
        const emailSummariesCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='email_summaries'").get();
        ...
        const esqCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='email_suggestion_queue'").get();
        if (esqCheck.count === 0) {
        ...
        const docsSpacesCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='docs_spaces'").get();
        ...
        const docsPagesCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='docs_pages'").get();
        ...
        const docsPagesFtsCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='docs_pages_fts'").get();
        ...
        const docsDraftsCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='docs_page_drafts'").get();
        ...
        const docsVersionsCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='docs_page_versions'").get();
        ...
        const docsTagsCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='docs_tags'").get();
        ...
        const docsLinksCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='docs_page_links'").get();
        ...
        const docsCommentsCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='docs_comments'").get();
        ...
        const docsProjCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='docs_page_projects'").get();
        ...
        const docsAttachCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='docs_attachments'").get();
        ...
        const docsTemplatesCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='docs_templates'").get();
        ...
        const draftTitle040 = db.prepare("SELECT count(*) as count FROM pragma_table_info('docs_page_drafts') WHERE name = 'title'").get();
        ...
        const maintLockCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='maintenance_locks'").get();
        ...
        const revCol = db.prepare("SELECT count(*) as c FROM pragma_table_info('skills') WHERE name='revision'").get();
        const arcCol = db.prepare("SELECT count(*) as c FROM pragma_table_info('skills') WHERE name='archived_at'").get();
        const hasRev = revCol.c > 0;
        const hasArc = arcCol.c > 0;
        const hasVerTable = db.prepare("SELECT count(*) as c FROM sqlite_master WHERE type='table' AND name='skill_versions'").get().c > 0;
        const hasAfterIns = db.prepare("SELECT count(*) as c FROM sqlite_master WHERE type='trigger' AND name='skill_versions_after_insert'").get().c > 0;
        const hasAfterUpd = db.prepare("SELECT count(*) as c FROM sqlite_master WHERE type='trigger' AND name='skill_versions_after_update'").get().c > 0;
        const hasBefUpd = db.prepare("SELECT count(*) as c FROM sqlite_master WHERE type='trigger' AND name='skill_versions_before_update'").get().c > 0;
        const hasBefDel = db.prepare("SELECT count(*) as c FROM sqlite_master WHERE type='trigger' AND name='skill_versions_before_delete'").get().c > 0;
        ...
        if (any042 && !all042) {
            const missing = [];
            if (!hasRev)
                missing.push("skills.revision column");
            if (!hasArc)
                missing.push("skills.archived_at column");
            if (!hasVerTable)
                missing.push("skill_versions table");
            if (!hasAfterIns)
                missing.push("skill_versions_after_insert trigger");
            if (!hasAfterUpd)
                missing.push("skill_versions_after_update trigger");
            if (!hasBefUpd)
                missing.push("skill_versions_before_update trigger");
            if (!hasBefDel)
                missing.push("skill_versions_before_delete trigger");
            throw new Error("Migration 042 is in a PARTIAL state. Some components exist but others are missing: " +
                missing.join(", ") + ". " +
        ...
        const lineageCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='skill_lineage'").get();
        if (lineageCheck.count === 0) {
            ...
            const lineageIdxCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='index' AND name='idx_skill_lineage_target'").get();
        ...
        const proposalsCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='skill_proposals'").get();
        if (proposalsCheck.count === 0) {
            ...
            for (const col of requiredCols) {
                const c = db.prepare("SELECT count(*) as c FROM pragma_table_info('skill_proposals') WHERE name = ?").get(col);
                if (c.c === 0)
                    missingCols.push(col);
            }
            if (missingCols.length > 0) {
                throw new Error("Migration 044 is in a PARTIAL state. The skill_proposals table exists but is missing required columns: " +
                    missingCols.join(", ") + ". " +
            ...
            const idxCheck = db.prepare("SELECT count(*) as c FROM sqlite_master WHERE type='index' AND name='idx_skill_proposals_candidate_uniq'").get();
            if (idxCheck.c === 0) {
                throw new Error("Migration 044 is in a PARTIAL state: skill_proposals table exists but idx_skill_proposals_candidate_uniq index is missing. " +
        ...
        const pipeline045Check = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='pipeline_events'").get();
        if (pipeline045Check.count > 0) {
            ...
            try {
                db.prepare("INSERT INTO pipeline_events (project_id, event_type, event_source, title, created_at) VALUES ('__migration_probe__', 'skill_created', 'synthesis', 'probe', ?)").run(new Date().toISOString());
        ...
        const vaultConfigCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='vault_config'").get();
        ...
        const backupRecordsCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='backup_records'").get();
        ...
        const ragSourcesCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='rag_sources'").get();
        ...
        const workspaceMigrationCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='project_migration_manifests'").get();
        ...
        const contextSourceCheck = db.prepare("SELECT count(*) as count FROM pragma_table_info('context_entries') WHERE name='source'").get();
        ...
        const ragSourcesSql = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='rag_sources'").get();
        const threadImportsCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='rag_thread_imports'").get();
        if (ragSourcesSql?.sql.includes("thread_import") || threadImportsCheck.count > 0) {
            const threadSourceCount = db.prepare("SELECT count(*) as count FROM rag_sources WHERE source_type = 'thread_import'").get().count;
            const threadImportCount = threadImportsCheck.count > 0
                ? db.prepare("SELECT count(*) as count FROM rag_thread_imports").get().count
                : 0;
        ...
        const agentsSql = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='agents'").get();
        ...
        const globalIndexCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='index' AND name='idx_projects_one_active_global'").get();
        const protectedSettingsCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='protected_settings'").get();
        if (globalIndexCheck.count === 0 || protectedSettingsCheck.count === 0) {
            if (globalIndexCheck.count === 0)
                reconcileDuplicateActiveGlobals(db);
        ...
        const agentMetadataCheck = db.prepare("SELECT count(*) as count FROM pragma_table_info('agents') WHERE name = 'metadata'").get();
        ...
        const brokerDeleteTriggerCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type = 'trigger' AND name = 'agents_broker_delete_protection'").get();
        ...
        const brokerRenameTriggerCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type = 'trigger' AND name = 'agents_broker_rename_protection'").get();
        ...
        const brokerConnectionIndependentTriggerCheck = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type = 'trigger' AND name = 'agents_broker_insert_template_protection'").get();
...
function reconcileDuplicateActiveGlobals(db) {
    const activeGlobals = db.prepare("SELECT id, name FROM projects WHERE is_global = 1 AND archived_at IS NULL ORDER BY name, created_at, id").all();
    if (activeGlobals.length <= 1)
        return;
    const canonical = activeGlobals.filter((project) => project.name === "global-default");
    if (canonical.length !== 1) {
        throw new Error("Migration 053 refused: multiple active global projects exist without an unambiguous global-default canonical project. Resolve the duplicate global designation before retrying.");
    }
    const now = new Date().toISOString();
    db.transaction(() => {
        db.prepare("UPDATE projects SET is_global = 0, updated_at = ? WHERE is_global = 1 AND archived_at IS NULL AND id <> ?").run(now, canonical[0].id);
    })();
 ...
 */
export function verifyAndRebuildSkillsFts(db) {
    // Verify virtual table exists
    const ftsTable = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='table' AND name='skills_fts'").get();
    if (ftsTable.count === 0) {
        throw new Error("skills_fts virtual table is missing after migration 041. " +
    ...
    for (const trig of triggers) {
        const found = db.prepare("SELECT count(*) as count FROM sqlite_master WHERE type='trigger' AND name = ?").get(trig);
        if (found.count === 0) {
            throw new Error(`FTS5 trigger '${trig}' is missing after migration 041. ` +
