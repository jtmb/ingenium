...
function getProviderConfig(account) {
 ...
 *    password auth (which triggers ImapFlow "No password configured").
 */
function buildAuth(account, auth) {
 ...
 */
export async function connectAccount(account, auth) {
...
function buildSearchCriteria(query) {
    ...
    if (query.answered)
        crit.answered = true;
    if (query.from)
        crit.from = query.from;
    if (query.to)
        crit.to = query.to;
    if (query.subject)
        crit.subject = query.subject;
    if (query.since)
        crit.since = query.since;
    if (query.before)
        crit.before = query.before;
    return crit;
}
 ...
 */
export async function listEmails(accountId, folder, page, limit, query, 
...
windowSize = 200) {
    ...
    const messages = [];
    for await (const msg of client.fetch(rangeStr, { envelope: true, uid: true, flags: true, source: true })) {
        const raw = msg.source?.toString("utf-8") ?? "";
        const parsed = await parseRawEmail(raw);
        parsed.uid = String(msg.uid);
        parsed.flags = msg.flags ? [...msg.flags] : [];
 ...
 */
export async function getEmail(accountId, folder, uid) {
    const client = getConnection(accountId);
    await client.mailboxOpen(folder);
 ...
 */
export async function searchEmails(accountId, folder, query) {
    const client = getConnection(accountId);
    await client.mailboxOpen(folder);
    const criteria = buildSearchCriteria(query);
 ...
 */
export async function setFlags(accountId, folder, uid, flags) {
    const client = getConnection(accountId);
    await client.mailboxOpen(folder);
 ...
 */
export async function deleteEmail(accountId, folder, uid) {
    const client = getConnection(accountId);
    await client.mailboxOpen(folder);
    await client.messageFlagsSet(uid, ["\\Deleted"], { uid: true });
    await client.messageDelete(uid, { uid: true });
