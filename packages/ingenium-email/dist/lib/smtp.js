 ...
 */
export async function createTransport(account, auth) {
 ...
 */
export async function sendEmail(account, auth, options) {
    try {
 ...
 */
export async function saveDraft(account, auth, options) {
...
function buildDraftRaw(account, options, messageId) {
    const lines = [];
    lines.push(`From: "${account.name}" <${account.email}>`);
    lines.push(`To: ${options.to.map((a) => addressString(a)).join(", ")}`);
    if (options.cc?.length)
        lines.push(`Cc: ${options.cc.map((a) => addressString(a)).join(", ")}`);
