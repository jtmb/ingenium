 ...
 */
export async function syncFolder(_projectId, accountId, folder, maxBatch = 200) {
    ...
    let resolve;
    let reject;
    const deferred = new Promise((res, rej) => { resolve = res; reject = rej; });
    ...
    (async () => {
        try {
            ...
            if (lastUid === 0) {
                ...
                const fetchedUids = [];
                for await (const msg of client.fetch(`${windowStart}:${total}`, { uid: true })) {
                ...
                try {
                    ...
                    const result = await client.search({ uid: `${lastUid + 1}:*` });
                    uids = (result === false ? [] : result).slice(0, maxBatch);
                }
                catch {
                ...
                try {
                    ...
                    for (const p of recentParsed) {
                        try {
                            ...
                            emailCache.upsertEmailBody(accountId, folder, String(p.uid), p.html ?? null, p.text ?? null, headersJson);
                            bodiesCached++;
                        }
                        catch {
 ...
 */
export async function backfillFolderBodies(_projectId, accountId, folder, limit = 50) {
    const projectId = getGlobalProjectId();
    const account = getAccount(projectId, accountId);
    if (!account) {
        ...
        for await (const msg of client.fetch(uids.map(Number), { source: true, uid: true }, { uid: true })) {
            try {
                ...
                if (parsed.body.html || parsed.body.text) {
                    ...
                    emailCache.upsertEmailBody(accountId, folder, String(msg.uid), parsed.body.html ?? null, parsed.body.text ?? null, headersJson);
                    backfilled++;
                }
            }
            catch {
 ...
 */
export async function syncAccountFolders(_projectId, accountId, opts) {
    const projectId = getGlobalProjectId();
    const account = getAccount(projectId, accountId);
    if (!account) {
    ...
    try {
        ...
        if (opts?.skipFresh) {
            ...
            if (staleFolders.length === 0) {
                ...
                return folders
                    ...
                    .map(f => {
                    const st = emailCache.getSyncState(accountId, f.path);
                    return { folder: f.path, synced: 0, total: Number(st.last_uid) > 0 ? 0 : f.totalMessages, skipped: true };
                });
            }
        }
        const results = [];
        ...
        logger.info("email", `Sync complete for ${account.email}: ` +
