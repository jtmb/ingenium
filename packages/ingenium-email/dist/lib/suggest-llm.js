 ...
 */
export async function getVoiceSamples(account, tokens, limit = 15, _signal) {
    try {
 ...
 */
export async function generateSmartReplies(targetEmail, voiceSamples, llmConfig, signal) {
    ...
    try {
        ...
        const valid = parsed
            .filter((s) => s && typeof s.tone === "string" && typeof s.subject === "string" && typeof s.body === "string")
            .map((s) => ({
...
// ── Prompt builder ─────────────────────────────────────────────────────────
function buildSmartReplyPrompt(targetEmail, voiceSamples) {
 ...
 */
export async function generateEmailSummary(emailBody, subject, llmConfig) {
    if (!llmConfig.endpoint || !llmConfig.model)
        return "";
 ...
 */
export async function reviewDraft(text, subject, llmConfig) {
    if (!llmConfig.endpoint || !llmConfig.model)
        return "";
