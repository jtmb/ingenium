...
import { isAuthenticationProviderError, providerErrorDiagnostic, sanitizeProviderError, } from "./provider-errors.js";
// ── Default settings ────────────────────────────────────────────────────────
 ...
 */
function isNoreplySender(fromAddr, fromName) {
 ...
 */
function enqueueTask(worker, task) {
    // Dedup: skip if same folder+type is already queued at same or higher priority
    const existing = worker.taskQueue.find(t => t.accountId === task.accountId &&
...
function getFolderState(worker, folder) {
...
function setFolderState(worker, folder, partial) {
 ...
 */
function isFolderFresh(accountId, folder) {
    ...
    for (let i = 0; i < 2; i++) {
        ...
        try {
            ...
            if (suggestions.length > 0) {
                emailCache.upsertEmailSuggestions(job.account_id, job.folder, job.uid, suggestions, llmConfig.model);
            }
            // Mark job complete
            emailSuggestionQueue.markJobComplete(job.id);
            logger.info("sync-engine", `Generated ${suggestions.length} smart replies for ${job.account_id}/${job.folder}/${job.uid}`);
        }
        catch (error) {
...
// ── Account worker ──────────────────────────────────────────────────────────
async function runAccountWorker(worker) {
            ...
            if (Date.now() - lastDeltaPoll >= GMAIL_POLL_INTERVAL_MS) {
                ...
                try {
                    ...
                    if (delta.fullResyncRequired) {
                        logger.info("sync-engine", `Full resync required for ${worker.email} (historyId ${historyId ?? "none"})`);
                        ...
                        if (delta.upserts.length > 0) {
                            for (const msg of delta.upserts) {
                                ...
                                if (msg.changeType === "added" && !isNoreplySender(msg.fromAddr, msg.fromName)) {
                                    const enqueued = emailSuggestionQueue.enqueueSuggestionJob(worker.accountId, msg.folder, msg.id);
                                    if (enqueued) {
                                        logger.info("sync-engine", `Enqueued smart-reply suggestion job for ${worker.accountId}/${msg.folder}/${msg.id}`);
                                    }
                                }
                            }
                            logger.info("sync-engine", `Delta upserts for ${worker.email}: ${delta.upserts.length} messages`);
                        }
                        // Deletes: log for now — per-UID cache deletion not yet supported
                        if (delta.deletes.length > 0) {
                            logger.info("sync-engine", `Delta deletes for ${worker.email}: ${delta.deletes.length} messages (cache cleanup not yet implemented)`);
                        }
...
async function executeSyncFolder(worker, accountId, folder) {
    ...
    try {
        ...
        const messages = await withWatchdog(worker.provider.listMessages(account, creds.tokens, folder, offlineWindow), TASK_WATCHDOG_MS, `listMessages ${worker.email}/${folder}`);
        ...
        logger.info("sync-engine", `Synced ${worker.email}/${folder}: ${cachedTotal} headers, ${bodyCount} bodies`);
...
async function executeBackfillBodies(worker, accountId, folder, boostUid) {
    ...
    try {
        ...
        for (let i = 0; i < uidsToFetch.length; i += BODY_BATCH_SIZE) {
            ...
            for (const uid of batch) {
                try {
                    ...
                    emailCache.upsertEmailBody(accountId, folder, uid, body.html ?? null, body.text ?? null, JSON.stringify({ attachments: body.attachments }));
                    backfilled++;
                }
                catch (error) {
        ...
        const newState = stillMissing.length === 0 && cachedTotal > 0
            ? "complete"
        ...
        logger.info("sync-engine", `Backfilled ${backfilled} bodies for ${worker.email}/${folder} ` +
 ...
 */
export function startEngine(_projectId) {
    ...
    if (engineState.running) {
        spawnWorkers(projectId).catch((error) => {
...
async function spawnWorkers(_projectId) {
    const pid = getProjectId();
    ...
    for (const acct of accounts) {
        ...
        if (engineState.workers.has(acct.id))
            continue;
        const worker = {
            accountId: acct.id,
            email: acct.email,
 ...
 */
export function stopAccountWorker(accountId) {
...
export async function stopEngine() {
    ...
    for (const [, worker] of engineState.workers) {
        ...
        if (worker.loopPromise) {
            stopPromises.push(worker.loopPromise.catch(() => {
 ...
 */
export function boostFolder(accountId, folder) {
    const worker = engineState.workers.get(accountId);
    if (!worker) {
 ...
 * Non-blocking, fire-and-forget.
 */
export function boostBody(accountId, folder, uid) {
    const worker = engineState.workers.get(accountId);
    if (!worker) {
