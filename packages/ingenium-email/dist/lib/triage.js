...
function categorizeEmail(subject, bodyText) {
...
function scorePriority(subject, bodyText, senderAddress, highPrioritySenders) {
    const combined = `${subject} ${bodyText}`.toLowerCase();
...
function suggestAction(priority, category, matchedSkills) {
    if (category === "automated" || category === "newsletter")
        return "ignore";
    if (priority === "high" && matchedSkills.length > 0)
        return "reply_now";
    if (priority === "medium" && matchedSkills.length > 0)
        return "draft";
    if (priority === "high")
        return "reply_now";
 ...
 */
export function loadEmailSkills(projectId) {
 ...
 */
export function loadHighPrioritySenders(projectId) {
 ...
 */
export async function triageEmails(projectId, accountId, limit = 20) {
    const emailSkills = loadEmailSkills(projectId);
    ...
    for (const msg of messages) {
        ...
        for (const skill of emailSkills) {
            ...
            const skillWords = new Set(skillContent
                .split(/\s+/)
                .filter((w) => w.length > 3));
            const emailWords = new Set(combined
                .split(/\s+/)
                .filter((w) => w.length > 3));
