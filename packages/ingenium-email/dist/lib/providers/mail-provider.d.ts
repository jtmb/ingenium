...
export interface MailProvider {
     ...
     */
    listMessages(account: EmailAccount, tokens: OAuthToken, folder: string, 
    /** Max results to return (offline window cap, e.g. 500). */
    window: number): Promise<CachedEmailWrite[]>;
    /**
     ...
     */
    changesSince(account: EmailAccount, tokens: OAuthToken, cursor: string | null): Promise<{
    ...
    getBody(account: EmailAccount, tokens: OAuthToken, id: string): Promise<{
        html?: string;
        text?: string;
        attachments: EmailAttachment[];
    }>;
    /** Fetch a single attachment by ID. */
    getAttachment(account: EmailAccount, tokens: OAuthToken, id: string, attachmentId: string): Promise<{
        data: Buffer;
        mimeType: string;
        filename: string;
    }>;
    /** Send an email. */
    send(account: EmailAccount, tokens: OAuthToken, options: SendOptions): Promise<void>;
    /** Modify folder/label assignments for a message (Gmail labels, IMAP move/flag). */
    modifyFolders(account: EmailAccount, tokens: OAuthToken, id: string, addFolder: string | null, removeFolder: string | null): Promise<void>;
}
