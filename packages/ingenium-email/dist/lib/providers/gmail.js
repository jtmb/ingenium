 ...
 */
import { listLabels as apiListLabels, listMessages as apiListMessages, batchGetMessages, getMessage, getHistory, getAttachment as apiGetAttachment, sendMessage as apiSendMessage, modifyMessage as apiModifyMessage, getProfile, } from "./gmail-api.js";
...
async function getLabelMap(token) {
    ...
    for (const label of labels) {
        ...
        if (SKIP_LABEL_IDS.has(label.id))
            continue;
...
async function findLabelId(token, folder) {
...
async function findLabelIds(token, folders) {
    const labelMap = await getLabelMap(token);
    const ids = [];
    for (const info of labelMap.values()) {
 ...
 */
async function mapMetadataToCachedEmail(msg, folder) {
    if (!msg.id)
        return null;
...
export const GmailProvider = {
    // ── listFolders ─────────────────────────────────────────────────────────
    async listFolders(account, _tokens) {
    ...
    // ── listMessages ────────────────────────────────────────────────────────
    async listMessages(account, _tokens, folder, window) {
        const token = await getFreshGmailToken(account.id);
    ...
    // ── changesSince ────────────────────────────────────────────────────────
    async changesSince(account, _tokens, cursor) {
        const token = await getFreshGmailToken(account.id);
        ...
        try {
            ...
            for (const entry of history.history ?? []) {
                ...
                for (const added of entry.messagesAdded ?? []) {
                    if (processedMessageIds.has(added.message?.id))
                        continue;
                ...
                for (const deleted of entry.messagesDeleted ?? []) {
                    if (processedMessageIds.has(deleted.message?.id))
                        continue;
                ...
                const labelChangeIds = new Set();
                for (const la of entry.labelsAdded ?? []) {
                    if (la.message?.id)
                        labelChangeIds.add(la.message.id);
                }
                for (const lr of entry.labelsRemoved ?? []) {
                    if (lr.message?.id)
                        labelChangeIds.add(lr.message.id);
                }
    ...
    // ── getBody ─────────────────────────────────────────────────────────────
    async getBody(account, _tokens, id) {
        const token = await getFreshGmailToken(account.id);
        ...
        const emailAttachments = result.attachments.map(a => ({
            partId: a.partId,
    ...
    // ── getAttachment ───────────────────────────────────────────────────────
    async getAttachment(account, _tokens, id, attachmentId) {
        const token = await getFreshGmailToken(account.id);
        ...
        const parts = msg.payload?.parts ?? (msg.payload ? [msg.payload] : []);
        const result = walkParts(parts);
        const found = result.attachments.find(a => a.attachmentId === attachmentId || a.partId === attachmentId);
    ...
    // ── send ────────────────────────────────────────────────────────────────
    async send(account, _tokens, options) {
        const token = await getFreshGmailToken(account.id);
    ...
    // ── modifyFolders ──────────────────────────────────────────────────────
    async modifyFolders(account, _tokens, id, addFolder, removeFolder) {
        const token = await getFreshGmailToken(account.id);
        let addLabelIds;
        let removeLabelIds;
        if (addFolder) {
 ...
 */
function buildRfc822(account, options) {
    const lines = [];
    // Headers
