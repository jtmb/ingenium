...
export function recoveryBootstrapCheckEnvironment(source: NodeJS.ProcessEnv = process.env): NodeJS.ProcessEnv {
  return {
...
export function recoveryBootstrapRestartEnvironment(
  ...
  return {
    ...recoveryBootstrapCheckEnvironment(source),
...
export function runRecoveryBootstrap(
  ...
  const options = {
    cwd: root,
    env: recoveryBootstrapCheckEnvironment(source),
  ...
  const result = runner(process.execPath, [canonicalProductionRestart], {
    ...options,
    env: recoveryBootstrapRestartEnvironment(productionRestartScriptSha256, source),
