...
export async function emailList(project, account, folder, page) {
...
export async function emailSearch(project, account, query, folder) {
...
export async function emailRead(project, account, uid, folder) {
...
export async function emailSend(project, account, to, subject, html, text, cc, bcc) {
    const body = { account, to: to.split(",").map((s) => ({ address: s.trim() })), subject };
    if (html)
        body.html = html;
    if (text)
        body.text = text;
    if (cc)
        body.cc = cc.split(",").map((s) => ({ address: s.trim() }));
    if (bcc)
        body.bcc = bcc.split(",").map((s) => ({ address: s.trim() }));
...
export async function emailDraft(project, account, to, subject, html) {
    const body = { account, to: to.split(",").map((s) => ({ address: s.trim() })), subject };
    if (html)
        body.html = html;
...
export async function emailFolders(project, account) {
    const res = await api.get("/emails/folders", { project, account });
    return { content: [{ type: "text", text: JSON.stringify(res.data) }] };
}
/** List connected email accounts */
export async function emailAccounts(project) {
...
export async function emailTriage(project, account, limit) {
...
export async function emailSuggestResponse(project, account, uid, folder) {
...
export async function emailDraftResponse(project, account, uid, folder) {
    const suggest = await api.get(`/emails/suggest/${uid}`, {
        project, account,
        folder: folder ?? "INBOX",
    });
    if (!suggest.data?.body) {
        return { content: [{ type: "text", text: JSON.stringify({ error: "No suggestion available" }) }] };
    }
    ...
    return { content: [{ type: "text", text: JSON.stringify({ draft: draft.data, suggestion: suggest.data }) }] };
}
/** List all learned email response patterns (skills with category 'email') */
export async function emailPatterns(project) {
    const res = await api.get("/skills", { project });
    const emailSkills = (res.data || []).filter((s) => s.category === "email");
    return { content: [{ type: "text", text: JSON.stringify(emailSkills) }] };
}
/** Start IMAP IDLE watcher for real-time email monitoring */
export async function emailWatchStart(project, account) {
...
export async function emailWatchStatus(project, account) {
