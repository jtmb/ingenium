...
Client-side OpenCode package for connecting OpenCode to the Ingenium MCP Server,
with lifecycle plugins and managed CLI tools.
...
`./plugin-specs`. The standalone `plugins/session-id-tui.ts` source is not in
...
## Managed OpenCode launcher

The package also exposes `ingenium-opencode`, which delegates to the managed
TUI launcher and replacement-first recovery path. It is separate from the
installed upstream `opencode` executable and does not build, test, or replace
the upstream session-export command. For `opencode export <session-id>` and the
verified OpenCode 1.18.30 framing/privacy caveats, see
[`OpenCode usage`](../../docs/usage/opencode.md#session-export-from-the-installed-cli).
