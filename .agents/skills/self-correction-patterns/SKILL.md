...
| You are about to run `find` or `grep` in `node_modules`, `.git`, `dist`, `build` | This command will scan thousands of files and hang the terminal | **STOP.** Use `tsc --noEmit`, `ls`, or read specific files instead. Check `local-models` skill. |
