...
> 🔴 **Reasoning model compatibility — LLM functions read both `content` and `reasoning_content`.** The suggest-llm.ts functions (`packages/ingenium-email/lib/suggest-llm.ts` lines 110, 232, 295) and the extraction engine (`packages/ingenium-core/lib/tools/extraction.ts` line 205) all fall back from `message.content` to `message.reasoning_content` when the former is empty. This is required for reasoning models (Qwen, DeepSeek-R1) that output the response body in `reasoning_content`. Any new LLM-calling function in the email or extraction domain must read both fields.
