...
token_file="${INGENIUM_API_TOKEN_FILE:-/run/ingenium-secrets/api/installation-api-token}"
...
opencode_password_file="${OPENCODE_SERVER_PASSWORD_FILE:?OPENCODE_SERVER_PASSWORD_FILE is required}"
email_encryption_key_file="${INGENIUM_EMAIL_ENCRYPTION_KEY_FILE:?INGENIUM_EMAIL_ENCRYPTION_KEY_FILE is required}"
...
if [ "$deployment_mode" = "control-plane" ]; then
  ...
  : "${INGENIUM_RUNTIME_SCHEME:?INGENIUM_RUNTIME_SCHEME is required in control-plane mode}"
...
exec env -i \
  ...
  INGENIUM_GLOBAL_CONFIG_PATH="/home/ingenium-opencode/.config/opencode" \
  ...
  OPENCODE_SERVER_URL="${OPENCODE_SERVER_URL:-http://127.0.0.1:4101}" \
  ...
  OPENCODE_DB_PATH="${OPENCODE_DB_PATH:-/home/ingenium-opencode/.local/share/opencode/opencode.db}" \
  ...
  INGENIUM_DASHBOARD_BOOTSTRAP_TOKEN_FILE="${INGENIUM_DASHBOARD_BOOTSTRAP_TOKEN_FILE:-/run/ingenium-secrets/api/dashboard-bootstrap-token}" \
  INGENIUM_EMAIL_ENCRYPTION_KEY_EMPTY_TRANSITION="${INGENIUM_EMAIL_ENCRYPTION_KEY_EMPTY_TRANSITION:-0}" \
  ...
  INGENIUM_RUNTIME_SCHEME="${INGENIUM_RUNTIME_SCHEME:-}" \
