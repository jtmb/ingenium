...
## 🔴 HARD RULE — Same-Turn Learning → Skill Pipeline

After every `ingenium_learning_log` call, the agent MUST immediately check:

1. Does this learning signal a new convention, repeated pattern, or missing dependency?
2. If YES: compose a full SKILL.md with name, description, rules, and code examples
3. Call `ingenium_skill_create(project="ingenium", name=..., description=..., content=...)` NOW — same turn
4. Call `ingenium_learning_log(project="ingenium", entry_type="skill", content="<skill name and purpose>", tags="skill")` to record the creation

Do NOT defer to session start. The skill must be queryable immediately.

**Examples:**
| Learning signal | entry_type | Skill to create |
|---------------|-----------|----------------|
| New dependency detected (e.g., prisma) | `pattern` | `prisma-conventions` with setup, queries, migrations patterns |
| Repeated convention across files (e.g., error wrapper pattern) | `pattern` | Name-SKILL.md with the extracted pattern |
| Framework version bump changes rules | `pattern` | Update existing framework skill |
| Missing test pattern discovered | `bug` | `testing-patterns` or update `useful-tests` |
