---
title: "Docker Image Authoring — Multi-Stage Builds, Security, and Best Practices"
impact: HIGH
impactDescription: "Ensures secure, lean, and cache-efficient container images"
tags: [docker, dockerfile, multi-stage, security, image-size]
---
...
- The `USER` directive must come AFTER any commands that need root
...
### 3. Layer Ordering for Cache Hits
...
- Package manager files (`package.json`, `go.mod`, `requirements.txt`) go BEFORE source
...
Every project with containers MUST have a `.dockerignore`.
...
Never use floating tags in production. Pin to a specific digest for reproducibility.
