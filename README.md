...
<p>
  <img src="https://img.shields.io/badge/skills-43%20total-green?style=flat-square" alt="43 total — skills + instructions + tools" />
...
| **Instructions** | `.agents/instructions/{name}/SKILL.md` (11 deployed + 1 source-only) | `skill-load`, `help`, `repo-context`, `update-skills`, `audit-skills`, `update-skill-index`, `generate-docs`, `write-docs`, `debugging-patterns`, `self-correction-patterns`, `local-model-commands` — invocable via `/` commands (plus source-only `thread-auto-context`) |
| **Tools** | `.agents/tools/{name}/SKILL.md` (5 files) | `chrome-devtools`, `playwright-mcp`, `gh-cli`, `github-issues`, `web-design-reviewer` — browser automation & GitHub operations |
...
graph TD
    ...
    C -->|Slash command| F[Load instruction from instructions/]
    C -->|Browser/GitHub task| G[Load tool from tools/]
...
| **Instructions** | `.agents/instructions/{name}/SKILL.md` | `/` slash command | Doc generation, repo context, update skills, debugging, recovery |
| **Tools** | `.agents/tools/{name}/SKILL.md` | `/` slash command | Browser automation (Chrome DevTools, Playwright), GitHub CLI, issue mgmt, web design review |
