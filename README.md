...
<p>
  <img src="https://img.shields.io/badge/MCP%20active%20tools-286-blue?style=flat-square" alt="286 active MCP tools" />
...
Plug any MCP-compatible client (OpenCode, Cline, Claude Desktop) into the extension package and gain 286 active tools — 284 `ingenium_`-prefixed server registrations plus 2 extension-registered tools across 32 baseline categories. The six coordination entries are retired and excluded from the active catalog; only historical data and negative fixtures retain their names. The active surface covers projects, skills, tasks, memory, mail, secrets, backups, jobs, agents, MCP servers, config, observations, personality traits, the synthesis pipeline, and the documentation workspace.
...
| **Skills** | 8 active canonical skills in split-skill format (SKILL.md + metadata.json + references/) under `.opencode/skills/`. Dashboard split-pane viewer with syntax-highlighted editing, collapsible file tree. Conflict-aware bidirectional sync via SHA-256 manifest |
| **Tasks** | Kanban board (todo → in_progress → review → done) with dependency tracking, priority scoring, full audit history, and 32 MCP tools |
...
The installed OpenCode CLI can provide a named-session snapshot, but a complete
export is not liveness, complete-history, deployment, or model/session proof.
The retained 2026-09-09 capture and framing caveats are documented in the
[CLI session-context audit](docs/reference/session-context-audit-2026-09-09.md).

The dedicated `ingenium-recovery-engineer` is mapped to
`openai/gpt-5.6-luna` / `max` with profile
...
inspection/checkpoint. The canonical Premium profile's permanent writer role
remains unchanged; Recovery has no
...
For this documentation migration, `ingenium-software-engineer-premium` is mapped
to `openai/gpt-5.6-sol` / `xhigh` and is review-only; all other mapped
operational agents use `openai/gpt-5.6-luna` / `max`. This temporary routing
status does not change the canonical Premium profile's permanent role.
...
Plugins ship inside the `@ingenium/extension` package. Reference the five registered plugins from your OpenCode config:
...
**Other MCP clients** — Point your client's `command` to `npx -y @ingenium/extension`. The active built-in surface contains 286 tools: 284 `ingenium_`-prefixed server registrations plus 2 extension tools. The six coordination entries are retired and excluded from the active catalog; only historical data and negative fixtures retain their names. No HTTP port, no network configuration.
...
│   └── ingenium-extension/   # Client-side npm package: MCP stdio server, plugins (observer, resource-sync, auto-observer, lifecycle, ponytail)
├── services/
│   ├── ingenium-api/         # Private Express REST API on :4096 behind the authenticated :4097 boundary. Sole database authority.
│   ├── ingenium-server/      # MCP stdio server — 284 active prefixed registrations. Calls API via HTTP. Zero DB access.
...
| Agent Behavior and Permissions | Respective [agent profiles](.opencode/agents/) |

## How Major Systems Work

### MCP Tool System
Built-in server tools use a single `ingenium_` prefix: `ingenium_<noun>_<verb>` (e.g., `ingenium_skill_list`, `ingenium_task_create`). Transport registrations in the MCP server are unprefixed (`skill_list`); the catalog maps them automatically to the `ingenium_` form. The two extension-registered plugin tools are `synthesize_observations` and `auto_observe_now`. The active built-in surface contains 286 tools: 284 `ingenium_`-prefixed server registrations plus those 2 extension tools. The six coordination entries are retired and excluded from the active catalog; only historical data and negative fixtures retain their names. It is defined in `packages/ingenium-core/lib/tools/mcp-tool-catalog.ts` and verified by a parity test that ensures every registered transport has a catalog entry and vice versa.
