...
| **Instructions** | `.agents/instructions/{name}/SKILL.md` (12 deployed) | `skill-load`, `help`, `repo-context`, `update-skills`, `audit-skills`, `update-skill-index`, `generate-docs`, `write-docs`, `debugging-patterns`, `self-correction-patterns`, `local-model-commands`, `thread-auto-context` — invocable via `/` commands |
