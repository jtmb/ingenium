...
| Extraction or synthesis reports learning authentication failure | The isolated learning credential was revoked or expired | Run `ingenium-coordination-reset reset-learning` through the pre-provisioned encrypted owner provider, then retry the learning operation. This does not replace or reconnect the general coordination credential. |
