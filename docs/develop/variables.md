...
| `INGENIUM_RUNTIME_WORKSPACE_BOOTSTRAP_MAP_FILE` | `/run/ingenium-bootstrap/runtime-workspaces.json` in Compose | runtime-manager control entrypoint | Read-only host workspace-map path. The root entrypoint requires an owner-only mode-`0600` regular file and copies it to the service-owned runtime path before dropping privileges. |
...
| `INGENIUM_RUNTIME_WORKSPACE_MAP_FILE` | `/run/ingenium-secrets/runtime-manager/workspaces.json` in Compose | private runtime manager | Service-owned mode-`0400` version-1 JSON map of workspace IDs to exact host and validation paths, copied from the protected bootstrap mount before privilege drop. |
