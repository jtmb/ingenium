 ...
 * - `/opencode`, `/vscode`, `/docs`, `/chat` → zero padding (immersive, fills entire viewport)
...
export default function MainContainer({ children }: { children: React.ReactNode }) {
  ...
  const isImmersive = pathname === "/opencode" || pathname === "/vscode" || pathname === "/docs" || pathname === "/chat";
  const fullWidth =
    pathname === "/mail" ||
    pathname === "/tasks" ||
    pathname.startsWith("/mail/");
  if (isImmersive) {
    return <main className="h-full min-h-0 p-0 min-w-0">{children}</main>;
  }
  return (
    <main className={
      pathname === "/tasks"
