...
export default function OrganizationsPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    ...
    {organizations.length > 1 && <Select aria-label="Organization" value={organization?.id} onChange={(event) => setSelected(event.target.value)}>{organizations.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select>}
    {!organization ? <p>No organizations are available.</p> : <>
      <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 hover:shadow-md transition-shadow"><h2 className="text-lg font-semibold">{organization.name}</h2><p className="text-sm text-[var(--color-text-secondary)]">Effective role: {capabilities.effectiveRole ?? organization.role}</p></section>
      <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 hover:shadow-md transition-shadow"><h2 className="text-lg font-semibold">Members</h2><ul className="mt-3 divide-y divide-[var(--color-border)]">{members.map((member) => <li key={member.userId} className="flex flex-col gap-3 py-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{member.displayName}</p><p className="break-all text-sm text-[var(--color-text-muted)]">{member.email}</p></div>{capabilities.canManageMembers ? <div className="flex flex-wrap gap-2"><Select aria-label={`Role for ${member.email}`} value={member.role} onChange={(event) => protect(() => api.organizations.setMemberRole(organization.id, member.userId, event.target.value))}>{["owner", "admin", "member", "viewer"].map((value) => <option key={value}>{value}</option>)}</Select><button className="rounded border border-[var(--color-border)] px-3 py-2 text-sm" onClick={() => protect(() => api.organizations.removeMember(organization.id, member.userId))}>Remove</button></div> : <span className="text-sm">{member.role}</span>}</li>)}</ul></section>
      {capabilities.canManageInvitations && <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 hover:shadow-md transition-shadow"><h2 className="text-lg font-semibold">Invitations</h2><form className="mt-4 grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto_auto]" onSubmit={(event) => { event.preventDefault(); protect(() => api.organizations.invite(organization.id, email, role).then((result) => { setEmail(""); return result; })); }}><div><label htmlFor="invite-email" className="text-sm font-medium">Email</label><input id="invite-email" className={authInput} type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} required /></div><Select aria-label="Invitation role" value={role} onChange={(event) => setRole(event.target.value)} wrapperClassName="self-end"><option value="admin">admin</option><option value="member">member</option><option value="viewer">viewer</option></Select><button className="self-end rounded bg-blue-600 px-4 py-2 text-white">Invite</button></form><ul className="mt-4 divide-y divide-[var(--color-border)]">{invitations.map((invitation) => <li key={invitation.id} className="flex flex-wrap items-center justify-between gap-3 py-3 text-sm"><span>{invitation.email} · {invitation.role} · {invitation.acceptedAt ? "accepted" : invitation.revokedAt ? "revoked" : "pending"}</span>{!invitation.acceptedAt && !invitation.revokedAt && <button className="rounded border border-[var(--color-border)] px-3 py-2" onClick={() => protect(() => api.organizations.revokeInvitation(organization.id, invitation.id))}>Revoke</button>}</li>)}</ul></section>}
