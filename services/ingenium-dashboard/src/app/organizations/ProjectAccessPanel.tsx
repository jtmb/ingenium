...
export default function ProjectAccessPanel({ organization }: { organization: OrganizationSummary }) {
  ...
  return <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 hover:shadow-md transition-shadow" aria-labelledby="project-access-title">
    <h2 id="project-access-title" className="text-lg font-semibold">Project access: {project}</h2>
    ...
    {status === "ready" && <>
      {capabilities.canManageProjectMembers && candidates.length > 0 && <form className="mt-4 grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto_auto]" onSubmit={(event) => { event.preventDefault(); if (selectedUser) mutate(() => api.organizations.setProjectMember(organization.id, project, selectedUser, role)); }}>
        <Select aria-label="Organization member" value={selectedUser} onChange={(event) => setSelectedUser(event.target.value)}><option value="">Select member</option>{candidates.map((member) => <option key={member.userId} value={member.userId}>{member.displayName} ({member.email})</option>)}</Select>
      ...
      {projectMembers.length === 0 ? <p className="mt-4 text-sm text-[var(--color-text-muted)]">No explicit project memberships.</p> : <ul className="mt-4 divide-y divide-[var(--color-border)]">{projectMembers.map((member) => <li key={member.userId} className="flex flex-col gap-3 py-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{member.displayName}</p><p className="break-all text-sm text-[var(--color-text-muted)]">{member.email}</p></div>{capabilities.canManageProjectMembers ? <div className="flex flex-wrap items-center gap-2"><Select aria-label={`Project role for ${member.email}`} value={member.role} onChange={(event) => { const nextRole = event.target.value; mutate(() => api.organizations.setProjectMember(organization.id, project, member.userId, nextRole)); }}><option value="editor">editor</option><option value="viewer">viewer</option></Select><button className="rounded border border-[var(--color-border)] px-3 py-2 text-sm" onClick={() => mutate(() => api.organizations.removeProjectMember(organization.id, project, member.userId))}>Remove</button></div> : <span className="text-sm text-[var(--color-text-secondary)]">{member.role}</span>}</li>)}</ul>}
