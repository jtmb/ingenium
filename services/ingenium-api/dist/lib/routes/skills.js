...
function versionToDto(v) {
    return {
        ...
        fileTree: safeJsonParse(v.file_tree, null),
        enabled: !!v.enabled,
...
function lineageToDto(l) {
    return {
        ...
        mergedFilePaths: safeJsonParse(l.merged_file_paths, []),
...
function camelizeProposedState(state) {
    const out = { ...state };
    const keyMap = {
...
function proposalToDto(p) {
    return {
        ...
        proposedState: camelizeProposedState(safeJsonParse(p.proposed_state, {})),
        evidence: safeJsonParse(p.evidence_json, []),
        observationIds: safeJsonParse(p.observation_ids, []),
...
function governanceErrorPayload(err) {
...
skillsRouter.post("/proposals", (req, res) => {
    ...
    if (!projectId)
        return;
    if (!checkSkillLock(req, res, projectId))
        return;
    const { proposalType, targetName, proposedState, sourceProjectId, sourceName, expectedRevision, evidence, observationIds, qualityScore, noveltyScore, contradictionFlag, candidateGroupKey, alwaysApply, targetSkillId, } = req.body || {};
    ...
    const coreProposedState = { ...proposedState };
    ...
    try {
        const proposal = skillGovernance.createProposal(projectId, proposalType, targetName, proposedStateJson, {
...
skillsRouter.post("/lineage", (req, res) => {
    ...
    try {
        const lineage = skillGovernance.createLineage(projectId, sourceProjectId, sourceName, targetSkillId, sourceHash, mergedFilePaths, tombstonePath, reason);
...
skillsRouter.post("/locks/acquire", (req, res) => {
    ...
    if (!acquired) {
        ...
        const conflictInfo = { retryAfterMs: Math.ceil(retryAfterMs) };
        if (existingLock) {
 ...
 */
function checkSkillLock(req, res, projectId) {
    const activeLock = maintenanceLocks.getLockStatus(LOCK_RESOURCE, projectId);
    if (!activeLock)
        return true;
    const requestToken = req.headers["x-ingenium-lock-token"] || "";
