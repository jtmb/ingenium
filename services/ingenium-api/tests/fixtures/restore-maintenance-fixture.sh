...
fixture_nonce="${INGENIUM_RESTORE_FIXTURE_NONCE:?INGENIUM_RESTORE_FIXTURE_NONCE is required}"
maintenance_node="${RESTORE_MAINTENANCE_NODE:?RESTORE_MAINTENANCE_NODE is required}"
maintenance_script="${RESTORE_MAINTENANCE_SCRIPT:?RESTORE_MAINTENANCE_SCRIPT is required}"
...
if ! env -i \
  ...
  "$maintenance_node" "$maintenance_script" --validate-fixture "$fixture_nonce"; then
  echo "restore fixture must use an approved disposable ingenium-restore-fixture-* root" >&2
  exit 64
fi
...
exec env -i \
  ...
  "$maintenance_node" "$maintenance_script" --fixture "$fixture_nonce"
