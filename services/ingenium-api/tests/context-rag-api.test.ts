...
describe("context RAG API", () => {
  ...
  it("imports a safely project-bound OpenCode session without logging or exposing raw text in list responses", async () => {
    ...
    mocks.getMessages.mockResolvedValue([
      {
        ...
        parts: [{ type: "text", text: "The bronze observatory needs durable context." }],
      },
      {
        info: { role: "assistant", time: { created: 1_700_000_010_000 } },
        parts: [{ type: "reasoning", text: "never import reasoning" }, { type: "text", text: "I will retain a source citation." }],
      },
    ]);
    const imported = await fetch(url("/imports/opencode-session"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ sessionId, directory, title: "Imported OpenCode session" }),
    });
    ...
    const unsafe = await fetch(url("/imports/opencode-session"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ sessionId, directory: "/workspaces/not-this-project" }),
    });
