...
// ── Settings ─────────────────────────────────────────────
server.registerTool("ingenium_setting_get", { description: "Get a setting value by key", inputSchema: { project: projectParam, key: z.string() } }, wrapHandler("ingenium_setting_get", async ({ project, key }) => settingGet(project, key)));
server.registerTool("ingenium_setting_set", { description: "Set a setting value", inputSchema: { project: projectParam, key: z.string(), value: z.string() } }, wrapHandler("ingenium_setting_set", async ({ project, key, value }) => settingSet(project, key, value)));
server.registerTool("ingenium_setting_test_llm", { description: "Test the configured synthesis LLM connection.", inputSchema: { project: projectParam } }, wrapHandler("ingenium_setting_test_llm", async ({ project }) => settingTestLlm(project)));
// ── Skills ──────────────────────────────────────────────
server.registerTool("ingenium_skill_list", { description: "List all skills for a project.", inputSchema: { project: projectParam } }, wrapHandler("ingenium_skill_list", async ({ project }) => skillTools.skillList(project)));
server.registerTool("ingenium_skill_load", { description: "Load a single skill by name.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler("ingenium_skill_load", async ({ project, name }) => skillTools.skillLoad(project, name)));
server.registerTool("ingenium_skill_search", { description: "Full-text search across skills.", inputSchema: { project: projectParam, query: z.string() } }, wrapHandler("ingenium_skill_search", async ({ project, query }) => skillTools.skillSearch(project, query)));
...
}, wrapHandler("ingenium_skill_create", async ({ project, name, description, content, category, tags, always_apply, files }) => skillTools.skillCreate(project, name, description, content, category, tags, always_apply, files)));
...
server.registerTool("ingenium_skill_delete", { description: "Delete a skill by name (archive-only semantics — soft-deletes to archived state, not permanent removal).", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler("ingenium_skill_delete", async ({ project, name }) => skillTools.skillDelete(project, name)));
...
server.registerTool("ingenium_skill_sync_all", {
...
server.registerTool("ingenium_skill_sync_all_preview", {
...
server.registerTool("ingenium_skill_archive", {
...
server.registerTool("ingenium_skill_restore", {
...
server.registerTool("ingenium_skill_list_archived", {
...
server.registerTool("ingenium_skill_versions", {
...
server.registerTool("ingenium_skill_rollback", {
...
server.registerTool("ingenium_skill_lineage_create", {
...
}, wrapHandler("ingenium_skill_lineage_create", async (args) => skillTools.skillLineageCreate(args.project, args.sourceProjectId, args.sourceName, args.targetSkillId, args.sourceHash, args.mergedFilePaths, args.tombstonePath, args.reason)));
server.registerTool("ingenium_skill_lineage_list", {
...
server.registerTool("ingenium_skill_proposal_create", {
...
server.registerTool("ingenium_skill_proposal_get", {
...
server.registerTool("ingenium_skill_proposal_submit", {
...
server.registerTool("ingenium_skill_proposal_approve", {
...
}, wrapHandler("ingenium_skill_proposal_approve", async ({ project, proposalId, reviewer, reason }) => skillTools.skillProposalApprove(project, proposalId, reviewer, reason)));
server.registerTool("ingenium_skill_proposal_reject", {
    description: "Reject a pending proposal. Reviewer is required; reason is optional.",
    inputSchema: { project: projectParam, proposalId: z.string().uuid(), reviewer: z.string(), reason: z.string().optional() },
}, wrapHandler("ingenium_skill_proposal_reject", async ({ project, proposalId, reviewer, reason }) => skillTools.skillProposalReject(project, proposalId, reviewer, reason)));
server.registerTool("ingenium_skill_proposal_rollback", {
    ...
    inputSchema: { project: projectParam, proposalId: z.string().uuid(), reviewer: z.string(), reason: z.string().optional() },
}, wrapHandler("ingenium_skill_proposal_rollback", async ({ project, proposalId, reviewer, reason }) => skillTools.skillProposalRollback(project, proposalId, reviewer, reason)));
// ── Observations ──────────────────────────────────────────
...
}, wrapHandler("ingenium_observe", async ({ project, observation_type, content, importance, source, context }) => observationTools.observationStore(project, observation_type, content, importance, source, context)));
...
server.registerTool("ingenium_observation_get", { description: "Get a single observation by ID.", inputSchema: { project: projectParam, observation_id: z.number() } }, wrapHandler("ingenium_observation_get", async ({ project, observation_id }) => observationTools.observationGet(project, observation_id)));
server.registerTool("ingenium_observation_update", {
...
server.registerTool("ingenium_observation_enrich", {
...
server.registerTool("ingenium_observation_delete", { description: "Hard delete a single observation by ID.", inputSchema: { project: projectParam, observation_id: z.number() } }, wrapHandler("ingenium_observation_delete", async ({ project, observation_id }) => observationTools.observationDelete(project, observation_id)));
server.registerTool("ingenium_observation_delete_by_source", {
...
server.registerTool("ingenium_personality_set_trait", {
...
}, wrapHandler("ingenium_personality_set_trait", async ({ project, trait_type, trait_value, display_label, confidence }) => personalityTools.personalitySetTrait(project, trait_type, trait_value, display_label, confidence)));
server.registerTool("ingenium_personality_trait_dismiss", { description: "Dismiss a trait (set as inactive without deleting).", inputSchema: { project: projectParam, trait_id: z.number() } }, wrapHandler("ingenium_personality_trait_dismiss", async ({ project, trait_id }) => personalityTools.personalityTraitDismiss(project, trait_id)));
server.registerTool("ingenium_personality_trait_disable", { description: "Disable a trait (harder deactivation).", inputSchema: { project: projectParam, trait_id: z.number() } }, wrapHandler("ingenium_personality_trait_disable", async ({ project, trait_id }) => personalityTools.personalityTraitDisable(project, trait_id)));
server.registerTool("ingenium_personality_trait_delete", { description: "Hard delete a single personality trait.", inputSchema: { project: projectParam, trait_id: z.number() } }, wrapHandler("ingenium_personality_trait_delete", async ({ project, trait_id }) => personalityTools.personalityTraitDelete(project, trait_id)));
server.registerTool("ingenium_personality_traits_delete_all", {
...
}, wrapHandler("ingenium_task_create", async ({ project, title, description, assigned_to }) => taskTools.taskCreate(project, title, description, assigned_to)));
...
server.registerTool("ingenium_task_get", { description: "Get a single task by ID.", inputSchema: { project: projectParam, task_id: z.string() } }, wrapHandler("ingenium_task_get", async ({ project, task_id }) => taskTools.taskGet(project, task_id)));
server.registerTool("ingenium_task_comments_list", { description: "List comments for a task.", inputSchema: { project: projectParam, task_id: z.string() } }, wrapHandler("ingenium_task_comments_list", async ({ project, task_id }) => taskTools.taskCommentsList(project, task_id)));
server.registerTool("ingenium_task_comment_edit", {
...
}, wrapHandler("ingenium_task_comment_edit", async ({ project, task_id, comment_id, body, actor }) => taskTools.taskCommentEdit(project, task_id, comment_id, body, actor)));
server.registerTool("ingenium_task_comment_react", {
...
}, wrapHandler("ingenium_task_comment_react", async ({ project, task_id, comment_id, reaction, actor }) => taskTools.taskCommentReact(project, task_id, comment_id, reaction, actor)));
server.registerTool("ingenium_task_links_list", { description: "List task links (blocks, relates_to, duplicates).", inputSchema: { project: projectParam, task_id: z.string() } }, wrapHandler("ingenium_task_links_list", async ({ project, task_id }) => taskTools.taskLinksList(project, task_id)));
server.registerTool("ingenium_task_link_delete", {
...
server.registerTool("ingenium_task_tree", { description: "Get the full task tree (parent + subtasks + linked tasks).", inputSchema: { project: projectParam, task_id: z.string() } }, wrapHandler("ingenium_task_tree", async ({ project, task_id }) => taskTools.taskTree(project, task_id)));
server.registerTool("ingenium_task_notification_read", { description: "Mark a notification as read.", inputSchema: { project: projectParam, notification_id: z.string() } }, wrapHandler("ingenium_task_notification_read", async ({ project, notification_id }) => taskTools.taskNotificationRead(project, notification_id)));
server.registerTool("ingenium_task_bulk_update", {
...
server.registerTool("ingenium_plan_search", { description: "Full-text search across context entries.", inputSchema: { project: projectParam, query: z.string() } }, wrapHandler("ingenium_plan_search", async ({ project, query }) => contextTools.planSearch(project, query)));
server.registerTool("ingenium_plan_list", { description: "List plan/context entries.", inputSchema: { project: projectParam } }, wrapHandler("ingenium_plan_list", async ({ project }) => contextTools.planList(project)));
// ── Projects ────────────────────────────────────────────
server.registerTool("ingenium_project_list", { description: "List all projects known to the Ingenium API.", inputSchema: {} }, wrapHandler("ingenium_project_list", async () => projectTools.projectList()));
server.registerTool("ingenium_project_init", { description: "Initialise a new project on the Ingenium API.", inputSchema: { name: z.string(), isGlobal: z.boolean().optional() } }, wrapHandler("ingenium_project_init", async ({ name, isGlobal }) => projectTools.projectInit(name, isGlobal)));
server.registerTool("ingenium_project_delete", { description: "Delete a project by name.", inputSchema: { name: z.string() } }, wrapHandler("ingenium_project_delete", async ({ name }) => projectTools.projectDelete(name)));
server.registerTool("ingenium_project_restore", { description: "Restore an archived project.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler("ingenium_project_restore", async ({ project, name }) => projectTools.projectRestore(project, name)));
server.registerTool("ingenium_project_list_archived", { description: "List archived projects.", inputSchema: { project: projectParam } }, wrapHandler("ingenium_project_list_archived", async ({ project }) => projectTools.projectListArchived(project)));
server.registerTool("ingenium_project_purge", { description: "Purge old projects.", inputSchema: { project: projectParam, retentionDays: z.number().optional() } }, wrapHandler("ingenium_project_purge", async ({ project, retentionDays }) => projectTools.projectPurge(project, retentionDays)));
server.registerTool("ingenium_project_set_global", { description: "Mark a project as global (or unmark).", inputSchema: { project: projectParam, name: z.string(), isGlobal: z.boolean() } }, wrapHandler("ingenium_project_set_global", async ({ project, name, isGlobal }) => projectTools.projectSetGlobal(project, name, isGlobal)));
server.registerTool("ingenium_project_rename", {
...
server.registerTool("ingenium_project_detail", { description: "Get detailed info about a project by name.", inputSchema: { name: z.string() } }, wrapHandler("ingenium_project_detail", async ({ name }) => projectTools.projectDetail(name)));
// ── Plugins ─────────────────────────────────────────────
server.registerTool("ingenium_plugin_list", { description: "List all plugins available for a project.", inputSchema: { project: projectParam } }, wrapHandler("ingenium_plugin_list", async ({ project }) => pluginTools.pluginList(project)));
server.registerTool("ingenium_plugin_get", { description: "Get a single plugin by name.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler("ingenium_plugin_get", async ({ project, name }) => pluginTools.pluginGet(project, name)));
server.registerTool("ingenium_plugin_enable", { description: "Enable a plugin for a project.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler("ingenium_plugin_enable", async ({ project, name }) => pluginTools.pluginEnable(project, name)));
server.registerTool("ingenium_plugin_disable", { description: "Disable a plugin for a project.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler("ingenium_plugin_disable", async ({ project, name }) => pluginTools.pluginDisable(project, name)));
...
server.registerTool("ingenium_plugin_delete", { description: "Delete a plugin from a project.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler("ingenium_plugin_delete", async ({ project, name }) => pluginTools.pluginDelete(project, name)));
...
server.registerTool("ingenium_plugin_source", { description: "Get a plugin's source content from disk.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler("ingenium_plugin_source", async ({ project, name }) => pluginTools.pluginSource(project, name)));
// ── Commands ─────────────────────────────────────────────
server.registerTool("ingenium_command_list", { description: "List all commands for a project.", inputSchema: { project: projectParam } }, wrapHandler("ingenium_command_list", async ({ project }) => commandTools.commandList(project)));
server.registerTool("ingenium_command_get", { description: "Get a command by name.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler("ingenium_command_get", async ({ project, name }) => commandTools.commandGet(project, name)));
server.registerTool("ingenium_command_create", {
...
server.registerTool("ingenium_command_update", {
...
server.registerTool("ingenium_command_delete", { description: "Delete a command.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler("ingenium_command_delete", async ({ project, name }) => commandTools.commandDelete(project, name)));
// ── Config ───────────────────────────────────────────────
...
}, wrapHandler("ingenium_config_get", async ({ project, type }) => configTools.configGet(project, type)));
...
}, wrapHandler("ingenium_config_set", async ({ project, type, content }) => configTools.configSet(project, type, content)));
server.registerTool("ingenium_config_sync", {
    description: "Sync config from disk to DB",
    inputSchema: { project: projectParam, type: z.enum(["project", "global"]).optional().default("project") },
}, wrapHandler("ingenium_config_sync", async ({ project, type }) => configTools.configSync(project, type)));
// ── Servers ─────────────────────────────────────────────
server.registerTool("ingenium_server_list", { description: "List all registered child MCP servers for a project.", inputSchema: { project: projectParam } }, wrapHandler("ingenium_server_list", async ({ project }) => serverTools.serverList(project)));
...
}, wrapHandler("ingenium_server_add", async ({ project, name, command, args, env, source }) => serverTools.serverAdd(project, name, command, args, env, source)));
...
server.registerTool("ingenium_server_update", {
...
server.registerTool("ingenium_server_sync_all", {
...
// ── Agents ──────────────────────────────────────────────
server.registerTool("ingenium_agent_list", { description: "List all agents for a project, optionally filtered by category.", inputSchema: { project: projectParam, category: z.string().optional() } }, wrapHandler("ingenium_agent_list", async ({ project, category }) => agentTools.agentList(project, category)));
server.registerTool("ingenium_agent_get", { description: "Get an agent by name.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler("ingenium_agent_get", async ({ project, name }) => agentTools.agentGet(project, name)));
...
server.registerTool("ingenium_agent_delete", { description: "Delete an agent by name.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler("ingenium_agent_delete", async ({ project, name }) => agentTools.agentDelete(project, name)));
...
}, wrapHandler("ingenium_email_send", async ({ project, account, to, subject, html, text, cc, bcc }) => emailTools.emailSend(project, account, to, subject, html, text, cc, bcc)));
...
server.registerTool("ingenium_email_account_create", {
...
}, wrapHandler("ingenium_email_account_create", async (args) => emailTools.emailAccountCreate(args.project, args.email, args.provider, args.authType, args.name, args.appPassword, args.imapHost, args.smtpHost, args.imapPort, args.smtpPort)));
server.registerTool("ingenium_email_account_delete", { description: "Delete an email account and clear its cached data.", inputSchema: { project: projectParam, account: z.string() } }, wrapHandler("ingenium_email_account_delete", async ({ project, account }) => emailTools.emailAccountDelete(project, account)));
server.registerTool("ingenium_email_account_test", { description: "Test IMAP connection for an account.", inputSchema: { project: projectParam, account: z.string() } }, wrapHandler("ingenium_email_account_test", async ({ project, account }) => emailTools.emailAccountTest(project, account)));
server.registerTool("ingenium_email_oauth_url", { description: "Get OAuth authorization URL — never returns tokens, only the URL.", inputSchema: { project: projectParam, provider: z.string() } }, wrapHandler("ingenium_email_oauth_url", async ({ project, provider }) => emailTools.emailOauthUrl(project, provider)));
server.registerTool("ingenium_email_oauth_exchange", {
...
}, wrapHandler("ingenium_email_oauth_exchange", async ({ project, provider, code, state, redirectUri, accountId }) => emailTools.emailOauthExchange(project, provider, code, state, redirectUri, accountId)));
server.registerTool("ingenium_email_summarize", {
...
server.registerTool("ingenium_email_review_draft", {
...
server.registerTool("ingenium_email_move", {
...
server.registerTool("ingenium_email_set_flags", {
...
server.registerTool("ingenium_email_delete", {
...
server.registerTool("ingenium_email_sync", {
...
server.registerTool("ingenium_email_sync_status", { description: "Get per-folder sync status from the engine.", inputSchema: { project: projectParam, account: z.string() } }, wrapHandler("ingenium_email_sync_status", async ({ project, account }) => emailTools.emailSyncStatus(project, account)));
server.registerTool("ingenium_email_watch_stop", { description: "Stop IMAP IDLE watcher.", inputSchema: { project: projectParam, account: z.string() } }, wrapHandler("ingenium_email_watch_stop", async ({ project, account }) => emailTools.emailWatchStop(project, account)));
server.registerTool("ingenium_email_attachment_get", {
...
}, wrapHandler("ingenium_email_attachment_get", async ({ project, account, uid, attachmentId, folder, outputPath }) => emailTools.emailAttachmentGet(project, account, uid, attachmentId, folder, outputPath)));
// ── Jobs ──────────────────────────────────────────────
...
server.registerTool("ingenium_job_get", { description: "Get a single job by ID.", inputSchema: { project: projectParam, job_id: z.string() } }, wrapHandler("ingenium_job_get", async ({ project, job_id }) => jobTools.jobGet(project, job_id)));
server.registerTool("ingenium_job_suggest", {
...
// ── Pipeline ───────────────────────────────────────────
server.registerTool("ingenium_pipeline_events", {
...
}, wrapHandler("ingenium_pipeline_events", async ({ project, source, type, limit, since }) => pipelineTools.pipelineEvents(project, source, type, limit, since)));
server.registerTool("ingenium_pipeline_timeline", {
...
}, wrapHandler("ingenium_pipeline_timeline", async ({ project, source, limit, since }) => pipelineTools.pipelineTimeline(project, source, limit, since)));
server.registerTool("ingenium_pipeline_event_log", {
...
}, wrapHandler("ingenium_pipeline_event_log", async (args) => pipelineTools.pipelineEventLog(args.project, args.eventType, args.eventSource, args.title, args.description, args.data, args.parentEventId, args.sessionId, args.importance)));
// ── Status ─────────────────────────────────────────────
server.registerTool("ingenium_service_status", { description: "Get overall service health — supervisord process states + application health.", inputSchema: { project: projectParam } }, wrapHandler("ingenium_service_status", async ({ project }) => statusTools.serviceStatus(project)));
server.registerTool("ingenium_service_application_detail", {
...
server.registerTool("ingenium_service_process_detail", { description: "Get single process detail via supervisor.getProcessInfo.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler("ingenium_service_process_detail", async ({ project, name }) => statusTools.serviceProcessDetail(project, name)));
server.registerTool("ingenium_service_process_logs", {
...
}, wrapHandler("ingenium_service_process_logs", async ({ project, name, offset, limit }) => statusTools.serviceProcessLogs(project, name, offset, limit)));
// ── Health ─────────────────────────────────────────────
server.registerTool("ingenium_health_check", { description: "API health check — returns status and uptime. No project param needed.", inputSchema: {} }, wrapHandler("ingenium_health_check", async () => healthCheck()));
// ── OpenCode ───────────────────────────────────────────
server.registerTool("ingenium_opencode_messages", {
...
// ── Dashboard ──────────────────────────────────────────
server.registerTool("ingenium_dashboard_summary", {
...
// ── Documentation ───────────────────────────────────────
server.registerTool("ingenium_docs_list_spaces", { description: "List all documentation spaces", inputSchema: { project: projectParam } }, wrapHandler("ingenium_docs_list_spaces", async ({ project }) => docsTools.docsListSpaces(project)));
server.registerTool("ingenium_docs_get_space", {
...
server.registerTool("ingenium_docs_create_space", {
...
server.registerTool("ingenium_docs_update_space", {
...
server.registerTool("ingenium_docs_delete_space", { description: "Delete a documentation space", inputSchema: { project: projectParam, id: z.number() } }, wrapHandler("ingenium_docs_delete_space", async ({ project, id }) => docsTools.docsDeleteSpace(project, id)));
server.registerTool("ingenium_docs_list_pages", {
...
server.registerTool("ingenium_docs_get_page_tree", { description: "Get the page tree for a space", inputSchema: { project: projectParam, spaceId: z.number() } }, wrapHandler("ingenium_docs_get_page_tree", async ({ project, spaceId }) => docsTools.docsGetPageTree(project, spaceId)));
server.registerTool("ingenium_docs_get_page", {
...
server.registerTool("ingenium_docs_create_page", {
...
server.registerTool("ingenium_docs_update_page", {
...
server.registerTool("ingenium_docs_delete_page", { description: "Archive (soft-delete) a documentation page", inputSchema: { project: projectParam, id: z.number() } }, wrapHandler("ingenium_docs_delete_page", async ({ project, id }) => docsTools.docsDeletePage(project, id)));
server.registerTool("ingenium_docs_restore_page", { description: "Restore an archived documentation page", inputSchema: { project: projectParam, id: z.number() } }, wrapHandler("ingenium_docs_restore_page", async ({ project, id }) => docsTools.docsRestorePage(project, id)));
server.registerTool("ingenium_docs_publish_page", {
...
server.registerTool("ingenium_docs_move_page", {
...
server.registerTool("ingenium_docs_search", {
...
server.registerTool("ingenium_docs_get_draft", { description: "Get the autosaved draft for a page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler("ingenium_docs_get_draft", async ({ project, pageId }) => docsTools.docsGetDraft(project, pageId)));
server.registerTool("ingenium_docs_save_draft", { description: "Save a draft for a documentation page", inputSchema: { project: projectParam, pageId: z.number(), content: z.string(), title: z.string().optional(), slug: z.string().optional(), baseRevision: z.number().optional() } }, wrapHandler("ingenium_docs_save_draft", async ({ project, pageId, content, title, slug, baseRevision }) => docsTools.docsSaveDraft(project, pageId, content, title, slug, baseRevision)));
server.registerTool("ingenium_docs_delete_draft", { description: "Delete the autosaved draft for a page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler("ingenium_docs_delete_draft", async ({ project, pageId }) => docsTools.docsDeleteDraft(project, pageId)));
server.registerTool("ingenium_docs_list_versions", { description: "List version history for a page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler("ingenium_docs_list_versions", async ({ project, pageId }) => docsTools.docsListVersions(project, pageId)));
server.registerTool("ingenium_docs_get_version", {
...
server.registerTool("ingenium_docs_restore_version", {
...
server.registerTool("ingenium_docs_list_comments", { description: "List comments on a documentation page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler("ingenium_docs_list_comments", async ({ project, pageId }) => docsTools.docsListComments(project, pageId)));
server.registerTool("ingenium_docs_create_comment", {
...
server.registerTool("ingenium_docs_resolve_comment", {
...
server.registerTool("ingenium_docs_delete_comment", {
...
server.registerTool("ingenium_docs_list_tags", { description: "List all tags", inputSchema: { project: projectParam } }, wrapHandler("ingenium_docs_list_tags", async ({ project }) => docsTools.docsListTags(project)));
server.registerTool("ingenium_docs_get_page_tags", { description: "Get tags for a page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler("ingenium_docs_get_page_tags", async ({ project, pageId }) => docsTools.docsGetPageTags(project, pageId)));
server.registerTool("ingenium_docs_add_tag", { description: "Add a tag to a page", inputSchema: { project: projectParam, pageId: z.number(), tagName: z.string() } }, wrapHandler("ingenium_docs_add_tag", async ({ project, pageId, tagName }) => docsTools.docsAddTag(project, pageId, tagName)));
server.registerTool("ingenium_docs_remove_tag", { description: "Remove a tag from a page", inputSchema: { project: projectParam, pageId: z.number(), tagId: z.number() } }, wrapHandler("ingenium_docs_remove_tag", async ({ project, pageId, tagId }) => docsTools.docsRemoveTag(project, pageId, tagId)));
server.registerTool("ingenium_docs_get_backlinks", { description: "Get pages linking to this page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler("ingenium_docs_get_backlinks", async ({ project, pageId }) => docsTools.docsGetBacklinks(project, pageId)));
server.registerTool("ingenium_docs_list_attachments", { description: "List attachments on a page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler("ingenium_docs_list_attachments", async ({ project, pageId }) => docsTools.docsListAttachments(project, pageId)));
server.registerTool("ingenium_docs_delete_attachment", {
...
server.registerTool("ingenium_docs_list_templates", { description: "List page templates", inputSchema: { project: projectParam } }, wrapHandler("ingenium_docs_list_templates", async ({ project }) => docsTools.docsListTemplates(project)));
server.registerTool("ingenium_docs_get_template", { description: "Get a template by ID", inputSchema: { project: projectParam, id: z.number() } }, wrapHandler("ingenium_docs_get_template", async ({ project, id }) => docsTools.docsGetTemplate(project, id)));
server.registerTool("ingenium_docs_create_template", {
...
server.registerTool("ingenium_docs_delete_template", { description: "Delete a template", inputSchema: { project: projectParam, id: z.number() } }, wrapHandler("ingenium_docs_delete_template", async ({ project, id }) => docsTools.docsDeleteTemplate(project, id)));
server.registerTool("ingenium_docs_update_template", {
...
server.registerTool("ingenium_docs_link_project", {
...
server.registerTool("ingenium_docs_unlink_project", {
...
server.registerTool("ingenium_docs_get_projects", { description: "Get projects linked to a page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler("ingenium_docs_get_projects", async ({ project, pageId }) => docsTools.docsGetProjects(project, pageId)));
server.registerTool("ingenium_docs_toggle_favorite", { description: "Toggle favorite status for a page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler("ingenium_docs_toggle_favorite", async ({ project, pageId }) => docsTools.docsToggleFavorite(project, pageId)));
server.registerTool("ingenium_docs_get_favorites", { description: "Get favorite pages", inputSchema: { project: projectParam } }, wrapHandler("ingenium_docs_get_favorites", async ({ project }) => docsTools.docsGetFavorites(project)));
server.registerTool("ingenium_docs_import_pages", {
...
server.registerTool("ingenium_docs_export_space", { description: "Export a space as JSON", inputSchema: { project: projectParam, spaceId: z.number() } }, wrapHandler("ingenium_docs_export_space", async ({ project, spaceId }) => docsTools.docsExportSpace(project, spaceId)));
server.registerTool("ingenium_docs_trash_list", { description: "List archived pages in a space's trash", inputSchema: { project: projectParam, spaceId: z.number() } }, wrapHandler("ingenium_docs_trash_list", async ({ project, spaceId }) => docsTools.docsListTrash(project, spaceId)));
server.registerTool("ingenium_docs_trash_purge", { description: "Permanently delete all archived pages in a space's trash", inputSchema: { project: projectParam, spaceId: z.number() } }, wrapHandler("ingenium_docs_trash_purge", async ({ project, spaceId }) => docsTools.docsPurgeTrash(project, spaceId)));
server.registerTool("ingenium_docs_attachment_download", {
...
server.registerTool("ingenium_docs_get_stats", { description: "Get documentation statistics", inputSchema: { project: projectParam } }, wrapHandler("ingenium_docs_get_stats", async ({ project }) => docsTools.docsGetStats(project)));
// ── Start ───────────────────────────────────────────────
/**
