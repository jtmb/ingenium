...
// ── Settings ─────────────────────────────────────────────
server.registerTool("setting_get", { description: "Get a setting value by key", inputSchema: { project: projectParam, key: z.string() } }, wrapHandler(C("setting_get"), async ({ project, key }) => settingGet(project, key)));
server.registerTool("setting_set", { description: "Set a setting value", inputSchema: { project: projectParam, key: z.string(), value: z.string() } }, wrapHandler(C("setting_set"), async ({ project, key, value }) => settingSet(project, key, value)));
server.registerTool("setting_test_llm", { description: "Test the configured synthesis LLM connection.", inputSchema: { project: projectParam } }, wrapHandler(C("setting_test_llm"), async ({ project }) => settingTestLlm(project)));
// ── Skills ──────────────────────────────────────────────
server.registerTool("skill_list", { description: "List all skills for a project.", inputSchema: { project: projectParam } }, wrapHandler(C("skill_list"), async ({ project }) => skillTools.skillList(project)));
server.registerTool("skill_load", { description: "Load a single skill by name.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("skill_load"), async ({ project, name }) => skillTools.skillLoad(project, name)));
server.registerTool("skill_search", { description: "Full-text search across skills.", inputSchema: { project: projectParam, query: z.string() } }, wrapHandler(C("skill_search"), async ({ project, query }) => skillTools.skillSearch(project, query)));
...
}, wrapHandler(C("skill_create"), async ({ project, name, description, content, category, tags, always_apply, files }) => skillTools.skillCreate(project, name, description, content, category, tags, always_apply, files)));
...
server.registerTool("skill_delete", { description: "Delete a skill by name (archive-only semantics — soft-deletes to archived state, not permanent removal).", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("skill_delete"), async ({ project, name }) => skillTools.skillDelete(project, name)));
server.registerTool("skill_enable", { description: "Enable a skill and sync to disk.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("skill_enable"), async ({ project, name }) => skillTools.skillEnable(project, name)));
server.registerTool("skill_disable", { description: "Disable a skill and remove from disk.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("skill_disable"), async ({ project, name }) => skillTools.skillDisable(project, name)));
server.registerTool("skill_sync", { description: "Sync a skill from its .md file on disk to the DB — edits made directly to the file are persisted.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("skill_sync"), async ({ project, name }) => skillTools.skillSync(project, name)));
server.registerTool("skill_consolidate", {
...
server.registerTool("skill_sync_all", {
    description: "Sync ALL skills disk→DB for a project. Returns per-skill status (created/updated/unchanged/skipped_archived/error). Use ?write_to_disk=true to also push DB skills back to disk.",
    inputSchema: { project: projectParam },
}, wrapHandler(C("skill_sync_all"), async ({ project }) => skillTools.skillSyncAll(project)));
server.registerTool("skill_sync_all_preview", {
...
server.registerTool("skill_restore", {
...
server.registerTool("skill_list_archived", {
...
server.registerTool("skill_versions", {
...
server.registerTool("skill_rollback", {
...
server.registerTool("skill_lineage_create", {
...
}, wrapHandler(C("skill_lineage_create"), async (args) => skillTools.skillLineageCreate(args.project, args.sourceProjectId, args.sourceName, args.targetSkillId, args.sourceHash, args.mergedFilePaths, args.tombstonePath, args.reason)));
server.registerTool("skill_lineage_list", {
...
server.registerTool("skill_proposal_create", {
...
}, wrapHandler(C("skill_proposal_create"), async (args) => skillTools.skillProposalCreate(args.project, args.proposalType, args.targetName, args.proposedState, args.sourceProjectId, args.sourceName, args.expectedRevision, args.evidence, args.observationIds, args.qualityScore, args.noveltyScore, args.contradictionFlag, args.candidateGroupKey, args.alwaysApply, args.targetSkillId)));
server.registerTool("skill_proposal_list", {
...
server.registerTool("skill_proposal_get", {
...
server.registerTool("skill_proposal_submit", {
...
server.registerTool("skill_proposal_approve", {
...
}, wrapHandler(C("skill_proposal_approve"), async ({ project, proposalId, reviewer, reason }) => skillTools.skillProposalApprove(project, proposalId, reviewer, reason)));
server.registerTool("skill_proposal_reject", {
    description: "Reject a pending proposal. Reviewer is required; reason is optional.",
    inputSchema: { project: projectParam, proposalId: z.string().uuid(), reviewer: z.string(), reason: z.string().optional() },
}, wrapHandler(C("skill_proposal_reject"), async ({ project, proposalId, reviewer, reason }) => skillTools.skillProposalReject(project, proposalId, reviewer, reason)));
server.registerTool("skill_proposal_rollback", {
    ...
    inputSchema: { project: projectParam, proposalId: z.string().uuid(), reviewer: z.string(), reason: z.string().optional() },
}, wrapHandler(C("skill_proposal_rollback"), async ({ project, proposalId, reviewer, reason }) => skillTools.skillProposalRollback(project, proposalId, reviewer, reason)));
// ── Observations ──────────────────────────────────────────
...
}, wrapHandler(C("observe"), async ({ project, observation_type, content, importance, source, context }) => observationTools.observationStore(project, observation_type, content, importance, source, context)));
server.registerTool("observation_search", {
...
server.registerTool("observation_list", {
...
server.registerTool("observation_stats", {
...
server.registerTool("observation_get", { description: "Get a single observation by ID.", inputSchema: { project: projectParam, observation_id: z.number() } }, wrapHandler(C("observation_get"), async ({ project, observation_id }) => observationTools.observationGet(project, observation_id)));
server.registerTool("observation_update", {
...
server.registerTool("observation_enrich", {
...
server.registerTool("observation_delete", { description: "Hard delete a single observation by ID.", inputSchema: { project: projectParam, observation_id: z.number() } }, wrapHandler(C("observation_delete"), async ({ project, observation_id }) => observationTools.observationDelete(project, observation_id)));
server.registerTool("observation_delete_by_source", {
...
// ── Personality ───────────────────────────────────────────
server.registerTool("personality", {
...
server.registerTool("personality_traits", {
...
server.registerTool("personality_set_trait", {
...
}, wrapHandler(C("personality_set_trait"), async ({ project, trait_type, trait_value, display_label, confidence }) => personalityTools.personalitySetTrait(project, trait_type, trait_value, display_label, confidence)));
server.registerTool("personality_trait_dismiss", { description: "Dismiss a trait (set as inactive without deleting).", inputSchema: { project: projectParam, trait_id: z.number() } }, wrapHandler(C("personality_trait_dismiss"), async ({ project, trait_id }) => personalityTools.personalityTraitDismiss(project, trait_id)));
server.registerTool("personality_trait_disable", { description: "Disable a trait (harder deactivation).", inputSchema: { project: projectParam, trait_id: z.number() } }, wrapHandler(C("personality_trait_disable"), async ({ project, trait_id }) => personalityTools.personalityTraitDisable(project, trait_id)));
server.registerTool("personality_trait_delete", { description: "Hard delete a single personality trait.", inputSchema: { project: projectParam, trait_id: z.number() } }, wrapHandler(C("personality_trait_delete"), async ({ project, trait_id }) => personalityTools.personalityTraitDelete(project, trait_id)));
server.registerTool("personality_traits_delete_all", {
...
// ── Synthesis ─────────────────────────────────────────────
server.registerTool("synthesis_run", {
...
server.registerTool("synthesis_status", {
...
server.registerTool("synthesis_cross_project", {
...
// ── Extraction ──────────────────────────────────────────
server.registerTool("extraction_run", {
...
}, wrapHandler(C("task_create"), async ({ project, title, description, assigned_to }) => taskTools.taskCreate(project, title, description, assigned_to)));
...
server.registerTool("task_complete", { description: "Mark a task as completed.", inputSchema: { project: projectParam, task_id: z.string() } }, wrapHandler(C("task_complete"), async ({ project, task_id }) => taskTools.taskComplete(project, task_id)));
server.registerTool("task_next", { description: "Get the highest-priority next task to work on.", inputSchema: { project: projectParam } }, wrapHandler(C("task_next"), async ({ project }) => taskTools.taskNext(project)));
...
}, wrapHandler(C("task_update"), async ({ project, task_id, fields }) => taskTools.taskUpdate(project, task_id, fields)));
server.registerTool("task_delete", { description: "Delete a task by ID.", inputSchema: { project: projectParam, task_id: z.string() } }, wrapHandler(C("task_delete"), async ({ project, task_id }) => taskTools.taskDelete(project, task_id)));
...
server.registerTool("task_comment", {
...
}, wrapHandler(C("task_comment"), async ({ project, task_id, author, body, parent_comment_id }) => taskTools.taskComment(project, task_id, author, body, parent_comment_id)));
...
}, wrapHandler(C("task_link"), async ({ project, task_id, linked_task_id, link_type }) => taskTools.taskLink(project, task_id, linked_task_id, link_type)));
server.registerTool("task_board_config_get", {
...
server.registerTool("task_board_config_set", {
...
}, wrapHandler(C("task_board_config_set"), async ({ project, columns, custom_field_defs }) => taskTools.taskBoardConfigSet(project, columns, custom_field_defs)));
server.registerTool("task_subtask_create", {
...
}, wrapHandler(C("task_subtask_create"), async ({ project, parent_id, title, description, assigned_to }) => taskTools.taskSubtaskCreate(project, parent_id, title, description, assigned_to)));
server.registerTool("task_notifications", {
...
}, wrapHandler(C("task_notifications"), async ({ project, recipient, unread }) => taskTools.taskNotifications(project, recipient, unread)));
server.registerTool("task_get", { description: "Get a single task by ID.", inputSchema: { project: projectParam, task_id: z.string() } }, wrapHandler(C("task_get"), async ({ project, task_id }) => taskTools.taskGet(project, task_id)));
server.registerTool("task_comments_list", { description: "List comments for a task.", inputSchema: { project: projectParam, task_id: z.string() } }, wrapHandler(C("task_comments_list"), async ({ project, task_id }) => taskTools.taskCommentsList(project, task_id)));
server.registerTool("task_comment_edit", {
...
}, wrapHandler(C("task_comment_edit"), async ({ project, task_id, comment_id, body, actor }) => taskTools.taskCommentEdit(project, task_id, comment_id, body, actor)));
server.registerTool("task_comment_react", {
...
}, wrapHandler(C("task_comment_react"), async ({ project, task_id, comment_id, reaction, actor }) => taskTools.taskCommentReact(project, task_id, comment_id, reaction, actor)));
server.registerTool("task_links_list", { description: "List task links (blocks, relates_to, duplicates).", inputSchema: { project: projectParam, task_id: z.string() } }, wrapHandler(C("task_links_list"), async ({ project, task_id }) => taskTools.taskLinksList(project, task_id)));
server.registerTool("task_link_delete", {
...
server.registerTool("task_tree", { description: "Get the full task tree (parent + subtasks + linked tasks).", inputSchema: { project: projectParam, task_id: z.string() } }, wrapHandler(C("task_tree"), async ({ project, task_id }) => taskTools.taskTree(project, task_id)));
server.registerTool("task_notification_read", { description: "Mark a notification as read.", inputSchema: { project: projectParam, notification_id: z.string() } }, wrapHandler(C("task_notification_read"), async ({ project, notification_id }) => taskTools.taskNotificationRead(project, notification_id)));
server.registerTool("task_bulk_update", {
...
server.registerTool("plan_search", { description: "Full-text search across context entries.", inputSchema: { project: projectParam, query: z.string() } }, wrapHandler(C("plan_search"), async ({ project, query }) => contextTools.planSearch(project, query)));
server.registerTool("plan_list", { description: "List plan/context entries.", inputSchema: { project: projectParam } }, wrapHandler(C("plan_list"), async ({ project }) => contextTools.planList(project)));
...
// ── Projects ────────────────────────────────────────────
server.registerTool("project_list", { description: "List all projects known to the Ingenium API.", inputSchema: {} }, wrapHandler(C("project_list"), async () => projectTools.projectList()));
server.registerTool("project_init", { description: "Initialise a new project on the Ingenium API.", inputSchema: { name: z.string(), isGlobal: z.boolean().optional() } }, wrapHandler(C("project_init"), async ({ name, isGlobal }) => projectTools.projectInit(name, isGlobal)));
server.registerTool("project_delete", { description: "Delete a project by name.", inputSchema: { name: z.string() } }, wrapHandler(C("project_delete"), async ({ name }) => projectTools.projectDelete(name)));
server.registerTool("project_restore", { description: "Restore an archived project.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("project_restore"), async ({ project, name }) => projectTools.projectRestore(project, name)));
server.registerTool("project_list_archived", { description: "List archived projects.", inputSchema: { project: projectParam } }, wrapHandler(C("project_list_archived"), async ({ project }) => projectTools.projectListArchived(project)));
server.registerTool("project_purge", { description: "Purge old projects.", inputSchema: { project: projectParam, retentionDays: z.number().optional() } }, wrapHandler(C("project_purge"), async ({ project, retentionDays }) => projectTools.projectPurge(project, retentionDays)));
server.registerTool("project_set_global", { description: "Mark a project as global (or unmark).", inputSchema: { project: projectParam, name: z.string(), isGlobal: z.boolean() } }, wrapHandler(C("project_set_global"), async ({ project, name, isGlobal }) => projectTools.projectSetGlobal(project, name, isGlobal)));
...
server.registerTool("project_detail", { description: "Get detailed info about a project by name.", inputSchema: { name: z.string() } }, wrapHandler(C("project_detail"), async ({ name }) => projectTools.projectDetail(name)));
server.registerTool("project_migrate_workspace", { description: "DB-only migration of the historical invalid /workspace project into global-default. Use dryRun first; never accesses filesystem /workspace.", inputSchema: { dryRun: z.boolean().optional() } }, wrapHandler(C("project_migrate_workspace"), async ({ dryRun }) => projectTools.projectMigrateWorkspace(dryRun)));
// ── Plugins ─────────────────────────────────────────────
server.registerTool("plugin_list", { description: "List all plugins available for a project.", inputSchema: { project: projectParam } }, wrapHandler(C("plugin_list"), async ({ project }) => pluginTools.pluginList(project)));
server.registerTool("plugin_get", { description: "Get a single plugin by name.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("plugin_get"), async ({ project, name }) => pluginTools.pluginGet(project, name)));
server.registerTool("plugin_enable", { description: "Enable a plugin for a project.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("plugin_enable"), async ({ project, name }) => pluginTools.pluginEnable(project, name)));
server.registerTool("plugin_disable", { description: "Disable a plugin for a project.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("plugin_disable"), async ({ project, name }) => pluginTools.pluginDisable(project, name)));
server.registerTool("plugin_create", {
...
server.registerTool("plugin_delete", { description: "Delete a plugin from a project.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("plugin_delete"), async ({ project, name }) => pluginTools.pluginDelete(project, name)));
server.registerTool("plugin_update", {
...
server.registerTool("plugin_source", { description: "Get a plugin's source content from disk.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("plugin_source"), async ({ project, name }) => pluginTools.pluginSource(project, name)));
// ── Commands ─────────────────────────────────────────────
server.registerTool("command_list", { description: "List all commands for a project.", inputSchema: { project: projectParam } }, wrapHandler(C("command_list"), async ({ project }) => commandTools.commandList(project)));
server.registerTool("command_get", { description: "Get a command by name.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("command_get"), async ({ project, name }) => commandTools.commandGet(project, name)));
server.registerTool("command_create", {
...
server.registerTool("command_update", {
...
server.registerTool("command_delete", { description: "Delete a command.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("command_delete"), async ({ project, name }) => commandTools.commandDelete(project, name)));
// ── Config ───────────────────────────────────────────────
...
}, wrapHandler(C("config_get"), async ({ project, type }) => configTools.configGet(project, type)));
...
}, wrapHandler(C("config_set"), async ({ project, type, content }) => configTools.configSet(project, type, content)));
server.registerTool("config_sync", {
    description: "Sync config from disk to DB",
    inputSchema: { project: projectParam, type: z.enum(["project", "global"]).optional().default("project") },
}, wrapHandler(C("config_sync"), async ({ project, type }) => configTools.configSync(project, type)));
// ── Servers ─────────────────────────────────────────────
server.registerTool("server_list", { description: "List all registered child MCP servers for a project.", inputSchema: { project: projectParam } }, wrapHandler(C("server_list"), async ({ project }) => serverTools.serverList(project)));
server.registerTool("server_add", {
...
}, wrapHandler(C("server_add"), async ({ project, name, command, args, env, source }) => serverTools.serverAdd(project, name, command, args, env, source)));
server.registerTool("server_remove", {
...
server.registerTool("server_update", {
...
server.registerTool("server_sync_all", {
...
// ── Agents ──────────────────────────────────────────────
server.registerTool("agent_list", { description: "List all agents for a project, optionally filtered by category.", inputSchema: { project: projectParam, category: z.string().optional() } }, wrapHandler(C("agent_list"), async ({ project, category }) => agentTools.agentList(project, category)));
server.registerTool("agent_get", { description: "Get an agent by name.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("agent_get"), async ({ project, name }) => agentTools.agentGet(project, name)));
...
server.registerTool("agent_delete", { description: "Delete an agent by name.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("agent_delete"), async ({ project, name }) => agentTools.agentDelete(project, name)));
server.registerTool("agent_enable", { description: "Enable an agent and write its .md file to disk.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("agent_enable"), async ({ project, name }) => agentTools.agentEnable(project, name)));
server.registerTool("agent_disable", { description: "Disable an agent and remove its .md file from disk.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("agent_disable"), async ({ project, name }) => agentTools.agentDisable(project, name)));
server.registerTool("agent_sync", { description: "Sync an agent from its .md file on disk to the DB — edits made directly to the file are persisted.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("agent_sync"), async ({ project, name }) => agentTools.agentSync(project, name)));
// ── Logs ──────────────────────────────────────────────
...
}, wrapHandler(C("logs_list"), async ({ project, source, level, since, limit }) => logTools.logsList(project, source, level, since, limit)));
server.registerTool("logs_sources", {
    description: "List active log sources (e.g., scheduler, api, auto-observer).",
    inputSchema: { project: projectParam },
}, wrapHandler(C("logs_sources"), async ({ project }) => logTools.logsSources(project)));
// ── Email ──────────────────────────────────────────────
...
server.registerTool("email_read", {
...
}, wrapHandler(C("email_send"), async ({ project, account, to, subject, html, text, cc, bcc }) => emailTools.emailSend(project, account, to, subject, html, text, cc, bcc)));
server.registerTool("email_draft", {
...
server.registerTool("email_triage", {
...
server.registerTool("email_suggest", {
...
server.registerTool("email_draft_response", {
...
server.registerTool("email_patterns", {
...
server.registerTool("email_watch_start", {
...
server.registerTool("email_watch_status", {
...
server.registerTool("email_account_create", {
...
}, wrapHandler(C("email_account_create"), async (args) => emailTools.emailAccountCreate(args.project, args.email, args.provider, args.authType, args.name, args.appPassword, args.imapHost, args.smtpHost, args.imapPort, args.smtpPort)));
server.registerTool("email_account_delete", { description: "Delete an email account and clear its cached data.", inputSchema: { project: projectParam, account: z.string() } }, wrapHandler(C("email_account_delete"), async ({ project, account }) => emailTools.emailAccountDelete(project, account)));
server.registerTool("email_account_test", { description: "Test IMAP connection for an account.", inputSchema: { project: projectParam, account: z.string() } }, wrapHandler(C("email_account_test"), async ({ project, account }) => emailTools.emailAccountTest(project, account)));
server.registerTool("email_oauth_url", { description: "Get OAuth authorization URL — never returns tokens, only the URL.", inputSchema: { project: projectParam, provider: z.string() } }, wrapHandler(C("email_oauth_url"), async ({ project, provider }) => emailTools.emailOauthUrl(project, provider)));
server.registerTool("email_oauth_exchange", {
...
}, wrapHandler(C("email_oauth_exchange"), async ({ project, provider, code, state, redirectUri, accountId }) => emailTools.emailOauthExchange(project, provider, code, state, redirectUri, accountId)));
server.registerTool("email_summarize", {
...
server.registerTool("email_review_draft", {
...
server.registerTool("email_sync", {
...
server.registerTool("email_sync_status", { description: "Get per-folder sync status from the engine.", inputSchema: { project: projectParam, account: z.string() } }, wrapHandler(C("email_sync_status"), async ({ project, account }) => emailTools.emailSyncStatus(project, account)));
server.registerTool("email_watch_stop", { description: "Stop IMAP IDLE watcher.", inputSchema: { project: projectParam, account: z.string() } }, wrapHandler(C("email_watch_stop"), async ({ project, account }) => emailTools.emailWatchStop(project, account)));
server.registerTool("email_attachment_get", {
...
}, wrapHandler(C("email_attachment_get"), async ({ project, account, uid, attachmentId, folder, outputPath }) => emailTools.emailAttachmentGet(project, account, uid, attachmentId, folder, outputPath)));
// ── Jobs ──────────────────────────────────────────────
server.registerTool("job_list", { description: "List all jobs for a project.", inputSchema: { project: projectParam } }, wrapHandler(C("job_list"), async ({ project }) => jobTools.jobList(project)));
server.registerTool("job_create", {
...
}, wrapHandler(C("job_create"), async ({ project, name, description, agent, prompt_template, schedule_cron, trigger_event, timeout_minutes }) => jobTools.jobCreate(project, name, description, agent, prompt_template, schedule_cron, trigger_event, timeout_minutes)));
server.registerTool("job_update", {
...
}, wrapHandler(C("job_update"), async ({ project, job_id, fields }) => jobTools.jobUpdate(project, job_id, fields)));
server.registerTool("job_delete", { description: "Delete a job by ID.", inputSchema: { project: projectParam, job_id: z.string() } }, wrapHandler(C("job_delete"), async ({ project, job_id }) => jobTools.jobDelete(project, job_id)));
server.registerTool("job_run", { description: "Manually trigger a job run.", inputSchema: { project: projectParam, job_id: z.string() } }, wrapHandler(C("job_run"), async ({ project, job_id }) => jobTools.jobRun(project, job_id)));
server.registerTool("job_runs", { description: "List all runs for a job.", inputSchema: { project: projectParam, job_id: z.string() } }, wrapHandler(C("job_runs"), async ({ project, job_id }) => jobTools.jobRuns(project, job_id)));
server.registerTool("job_run_logs", {
...
server.registerTool("job_run_cancel", { description: "Cancel a running job.", inputSchema: { project: projectParam, run_id: z.string() } }, wrapHandler(C("job_run_cancel"), async ({ project, run_id }) => jobTools.jobRunCancel(project, run_id)));
server.registerTool("job_get", { description: "Get a single job by ID.", inputSchema: { project: projectParam, job_id: z.string() } }, wrapHandler(C("job_get"), async ({ project, job_id }) => jobTools.jobGet(project, job_id)));
server.registerTool("job_suggest", {
...
// ── Pipeline ───────────────────────────────────────────
server.registerTool("pipeline_events", {
...
}, wrapHandler(C("pipeline_events"), async ({ project, source, type, limit, since }) => pipelineTools.pipelineEvents(project, source, type, limit, since)));
server.registerTool("pipeline_timeline", {
...
}, wrapHandler(C("pipeline_timeline"), async ({ project, source, limit, since }) => pipelineTools.pipelineTimeline(project, source, limit, since)));
server.registerTool("pipeline_event_log", {
...
}, wrapHandler(C("pipeline_event_log"), async (args) => pipelineTools.pipelineEventLog(args.project, args.eventType, args.eventSource, args.title, args.description, args.data, args.parentEventId, args.sessionId, args.importance)));
// ── Status ─────────────────────────────────────────────
server.registerTool("service_status", { description: "Get overall service health — supervisord process states + application health.", inputSchema: { project: projectParam } }, wrapHandler(C("service_status"), async ({ project }) => statusTools.serviceStatus(project)));
server.registerTool("service_application_detail", {
...
server.registerTool("service_process_detail", { description: "Get single process detail via supervisor.getProcessInfo.", inputSchema: { project: projectParam, name: z.string() } }, wrapHandler(C("service_process_detail"), async ({ project, name }) => statusTools.serviceProcessDetail(project, name)));
server.registerTool("service_process_logs", {
...
}, wrapHandler(C("service_process_logs"), async ({ project, name, offset, limit }) => statusTools.serviceProcessLogs(project, name, offset, limit)));
// ── Health ─────────────────────────────────────────────
server.registerTool("health_check", { description: "API health check — returns status and uptime. No project param needed.", inputSchema: {} }, wrapHandler(C("health_check"), async () => healthCheck()));
// ── OpenCode ───────────────────────────────────────────
...
// ── Dashboard ──────────────────────────────────────────
server.registerTool("dashboard_summary", {
...
// ── Documentation ───────────────────────────────────────
server.registerTool("docs_list_spaces", { description: "List all documentation spaces", inputSchema: { project: projectParam } }, wrapHandler(C("docs_list_spaces"), async ({ project }) => docsTools.docsListSpaces(project)));
server.registerTool("docs_get_space", {
...
server.registerTool("docs_create_space", {
...
server.registerTool("docs_update_space", {
...
server.registerTool("docs_delete_space", { description: "Delete a documentation space", inputSchema: { project: projectParam, id: z.number() } }, wrapHandler(C("docs_delete_space"), async ({ project, id }) => docsTools.docsDeleteSpace(project, id)));
server.registerTool("docs_list_pages", {
...
server.registerTool("docs_get_page_tree", { description: "Get the page tree for a space", inputSchema: { project: projectParam, spaceId: z.number() } }, wrapHandler(C("docs_get_page_tree"), async ({ project, spaceId }) => docsTools.docsGetPageTree(project, spaceId)));
server.registerTool("docs_get_page", {
...
server.registerTool("docs_create_page", {
...
server.registerTool("docs_update_page", {
...
server.registerTool("docs_delete_page", { description: "Archive (soft-delete) a documentation page", inputSchema: { project: projectParam, id: z.number() } }, wrapHandler(C("docs_delete_page"), async ({ project, id }) => docsTools.docsDeletePage(project, id)));
server.registerTool("docs_restore_page", { description: "Restore an archived documentation page", inputSchema: { project: projectParam, id: z.number() } }, wrapHandler(C("docs_restore_page"), async ({ project, id }) => docsTools.docsRestorePage(project, id)));
server.registerTool("docs_publish_page", {
...
server.registerTool("docs_move_page", {
...
server.registerTool("docs_get_draft", { description: "Get the autosaved draft for a page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler(C("docs_get_draft"), async ({ project, pageId }) => docsTools.docsGetDraft(project, pageId)));
server.registerTool("docs_save_draft", { description: "Save a draft for a documentation page", inputSchema: { project: projectParam, pageId: z.number(), content: z.string(), title: z.string().optional(), slug: z.string().optional(), baseRevision: z.number().optional() } }, wrapHandler(C("docs_save_draft"), async ({ project, pageId, content, title, slug, baseRevision }) => docsTools.docsSaveDraft(project, pageId, content, title, slug, baseRevision)));
server.registerTool("docs_delete_draft", { description: "Delete the autosaved draft for a page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler(C("docs_delete_draft"), async ({ project, pageId }) => docsTools.docsDeleteDraft(project, pageId)));
server.registerTool("docs_list_versions", { description: "List version history for a page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler(C("docs_list_versions"), async ({ project, pageId }) => docsTools.docsListVersions(project, pageId)));
server.registerTool("docs_get_version", {
...
server.registerTool("docs_restore_version", {
...
server.registerTool("docs_list_comments", { description: "List comments on a documentation page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler(C("docs_list_comments"), async ({ project, pageId }) => docsTools.docsListComments(project, pageId)));
server.registerTool("docs_create_comment", {
...
server.registerTool("docs_resolve_comment", {
...
server.registerTool("docs_delete_comment", {
...
server.registerTool("docs_list_tags", { description: "List all tags", inputSchema: { project: projectParam } }, wrapHandler(C("docs_list_tags"), async ({ project }) => docsTools.docsListTags(project)));
server.registerTool("docs_get_page_tags", { description: "Get tags for a page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler(C("docs_get_page_tags"), async ({ project, pageId }) => docsTools.docsGetPageTags(project, pageId)));
server.registerTool("docs_add_tag", { description: "Add a tag to a page", inputSchema: { project: projectParam, pageId: z.number(), tagName: z.string() } }, wrapHandler(C("docs_add_tag"), async ({ project, pageId, tagName }) => docsTools.docsAddTag(project, pageId, tagName)));
server.registerTool("docs_remove_tag", { description: "Remove a tag from a page", inputSchema: { project: projectParam, pageId: z.number(), tagId: z.number() } }, wrapHandler(C("docs_remove_tag"), async ({ project, pageId, tagId }) => docsTools.docsRemoveTag(project, pageId, tagId)));
server.registerTool("docs_get_backlinks", { description: "Get pages linking to this page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler(C("docs_get_backlinks"), async ({ project, pageId }) => docsTools.docsGetBacklinks(project, pageId)));
server.registerTool("docs_list_attachments", { description: "List attachments on a page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler(C("docs_list_attachments"), async ({ project, pageId }) => docsTools.docsListAttachments(project, pageId)));
server.registerTool("docs_delete_attachment", {
...
server.registerTool("docs_list_templates", { description: "List page templates", inputSchema: { project: projectParam } }, wrapHandler(C("docs_list_templates"), async ({ project }) => docsTools.docsListTemplates(project)));
server.registerTool("docs_get_template", { description: "Get a template by ID", inputSchema: { project: projectParam, id: z.number() } }, wrapHandler(C("docs_get_template"), async ({ project, id }) => docsTools.docsGetTemplate(project, id)));
server.registerTool("docs_create_template", {
...
server.registerTool("docs_delete_template", { description: "Delete a template", inputSchema: { project: projectParam, id: z.number() } }, wrapHandler(C("docs_delete_template"), async ({ project, id }) => docsTools.docsDeleteTemplate(project, id)));
server.registerTool("docs_update_template", {
...
server.registerTool("docs_link_project", {
...
server.registerTool("docs_unlink_project", {
...
server.registerTool("docs_get_projects", { description: "Get projects linked to a page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler(C("docs_get_projects"), async ({ project, pageId }) => docsTools.docsGetProjects(project, pageId)));
server.registerTool("docs_toggle_favorite", { description: "Toggle favorite status for a page", inputSchema: { project: projectParam, pageId: z.number() } }, wrapHandler(C("docs_toggle_favorite"), async ({ project, pageId }) => docsTools.docsToggleFavorite(project, pageId)));
server.registerTool("docs_get_favorites", { description: "Get favorite pages", inputSchema: { project: projectParam } }, wrapHandler(C("docs_get_favorites"), async ({ project }) => docsTools.docsGetFavorites(project)));
server.registerTool("docs_import_pages", {
...
server.registerTool("docs_export_space", { description: "Export a space as JSON", inputSchema: { project: projectParam, spaceId: z.number() } }, wrapHandler(C("docs_export_space"), async ({ project, spaceId }) => docsTools.docsExportSpace(project, spaceId)));
server.registerTool("docs_trash_list", { description: "List archived pages in a space's trash", inputSchema: { project: projectParam, spaceId: z.number() } }, wrapHandler(C("docs_trash_list"), async ({ project, spaceId }) => docsTools.docsListTrash(project, spaceId)));
server.registerTool("docs_trash_purge", { description: "Permanently delete all archived pages in a space's trash", inputSchema: { project: projectParam, spaceId: z.number() } }, wrapHandler(C("docs_trash_purge"), async ({ project, spaceId }) => docsTools.docsPurgeTrash(project, spaceId)));
server.registerTool("docs_attachment_download", {
...
server.registerTool("docs_get_stats", { description: "Get documentation statistics", inputSchema: { project: projectParam } }, wrapHandler(C("docs_get_stats"), async ({ project }) => docsTools.docsGetStats(project)));
// ── RAG (Retrieval-Augmented Generation) ─────────────────
server.registerTool("docs_search_semantic", {
...
server.registerTool("docs_ingest", {
...
server.registerTool("docs_rag_sources_list", {
...
server.registerTool("docs_rag_source_get", {
...
server.registerTool("docs_rag_source_delete", {
...
server.registerTool("docs_rag_reingest", {
...
server.registerTool("docs_rag_stats", {
...
// ── Providers ──────────────────────────────────────────
server.registerTool("provider_list", { description: "List all available LLM providers from OpenCode", inputSchema: { project: projectParam } }, wrapHandler(C("provider_list"), async ({ project }) => providerTools.providerList(project)));
server.registerTool("provider_connect", {
...
}, wrapHandler(C("provider_connect"), async ({ project, providerId, key, metadata }) => providerTools.providerConnect(project, providerId, key, metadata)));
server.registerTool("provider_disconnect", { description: "Disconnect a provider", inputSchema: { project: projectParam, providerId: z.string() } }, wrapHandler(C("provider_disconnect"), async ({ project, providerId }) => providerTools.providerDisconnect(project, providerId)));
server.registerTool("provider_status", { description: "Get provider connection status (keys redacted)", inputSchema: { project: projectParam } }, wrapHandler(C("provider_status"), async ({ project }) => providerTools.providerStatus(project)));
// ── Vault ──────────────────────────────────────────────
server.registerTool("vault_status", { description: "Get vault status (sealed/unsealed/error).", inputSchema: { project: projectParam } }, wrapHandler(C("vault_status"), async ({ project }) => vaultTools.vaultStatus(project)));
server.registerTool("vault_unseal", {
...
server.registerTool("vault_seal", { description: "Seal (lock) the vault.", inputSchema: { project: projectParam } }, wrapHandler(C("vault_seal"), async ({ project }) => vaultTools.vaultSeal(project)));
server.registerTool("vault_item_list", {
...
server.registerTool("vault_item_create", {
...
}, wrapHandler(C("vault_item_create"), async ({ project, name, type, value, folderId, tags, urls, username }) => vaultTools.vaultItemCreate(project, name, type, value, folderId, tags, urls, username)));
server.registerTool("vault_item_get", {
...
server.registerTool("vault_item_update", {
...
server.registerTool("vault_item_delete", {
...
server.registerTool("vault_password_gen", {
...
server.registerTool("vault_audit_list", {
...
server.registerTool("backup_get", {
...
server.registerTool("backup_download", {
...
}, wrapHandler(C("backup_download"), async ({ project, backupId, outputPath }) => backupTools.backupDownload(project, backupId, outputPath)));
server.registerTool("backup_delete", {
...
server.registerTool("backup_restore_preview", {
    description: "Preview what a restore would do without executing it.",
    inputSchema: { project: projectParam, backupId: z.string() },
}, wrapHandler(C("backup_restore_preview"), async ({ project, backupId }) => backupTools.backupRestorePreview(project, backupId)));
server.registerTool("backup_restore_start", {
    description: "Start a restore operation. Requires confirm=true to proceed.",
    inputSchema: { project: projectParam, backupId: z.string() },
}, wrapHandler(C("backup_restore_start"), async ({ project, backupId }) => backupTools.backupRestoreStart(project, backupId)));
server.registerTool("backup_restore_status", {
    description: "Get the status of a restore operation by job ID.",
    inputSchema: { project: projectParam, jobId: z.string() },
}, wrapHandler(C("backup_restore_status"), async ({ project, jobId }) => backupTools.backupRestoreStatus(project, jobId)));
server.registerTool("backup_schedule_get", {
...
server.registerTool("backup_schedule_set", {
    description: "Set/update the backup schedule configuration.",
    inputSchema: { project: projectParam, config: z.record(z.unknown()) },
}, wrapHandler(C("backup_schedule_set"), async ({ project, config }) => backupTools.backupScheduleSet(project, config)));
// ── Start ───────────────────────────────────────────────
/**
