...
export async function contextGet(project: string, id: number) { const res = await api.get(`/context/${id}`, { project }); return textResult(res.data); }
export async function contextUpdate(project: string, id: number, fields: Record<string, unknown>) { const res = await api.patch(`/context/${id}`, fields, { project }); return textResult(res.data); }
...
export async function contextBatch(project: string, ids: number[]) { const res = await api.post("/context/batch", { ids }, { project }); return textResult(res.data); }
