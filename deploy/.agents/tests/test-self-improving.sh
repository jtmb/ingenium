...
" 2>/dev/null || echo "")
    ...
    for dep in $deps; do
        ...
        if ! $covered; then
            ...
            local mentioned
            mentioned=$(grep -rli "$dep" "$SKILLS_DIR" 2>/dev/null | wc -l || echo 0)
    ...
    for expected_gap in "solidjs" "astro" "bullmq"; do
        local mentions
        mentions=$(grep -rli "$expected_gap" "$SKILLS_DIR" 2>/dev/null | wc -l || echo 0)
...
test_missing_coverage() {
    ...
    for gap in "${KNOWN_GAPS[@]}"; do
        local mentions
        mentions=$(grep -rli "\\*\\.${gap}" "$SKILLS_DIR" 2>/dev/null | wc -l || echo 0)
        if [[ "$mentions" -eq 0 ]]; then
