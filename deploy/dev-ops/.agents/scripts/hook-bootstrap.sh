...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help) echo "Usage: hook-bootstrap.sh [--framework FW] [--no-cache] [--project-root DIR]"; exit 0 ;;
        *) shift ;;
    esac
done
...
if [[ -d "$CACHE_DIR/.git" ]]; then
    git -C "$CACHE_DIR" fetch --quiet origin "$BOOTSTRAP_REPO_REF" 2>/dev/null || true
    git -C "$CACHE_DIR" checkout --quiet "origin/$BOOTSTRAP_REPO_REF" 2>/dev/null || true
    git -C "$CACHE_DIR" pull --quiet --ff-only origin "$BOOTSTRAP_REPO_REF" 2>/dev/null || true
else
    if ! git clone --quiet --depth 1 --branch "$BOOTSTRAP_REPO_REF" \
        "$BOOTSTRAP_REPO_URL" "$CACHE_DIR" 2>/dev/null; then
        echo "hook-bootstrap: ERROR: Failed to clone $BOOTSTRAP_REPO_URL" >&2
        exit 0
    fi
fi
...
# ── Bootstrap ─────────────────────────────────────────────
if ! "$CACHE_DIR/.agents/scripts/bootstrap.sh" \
    --auto \
    --framework "$FRAMEWORK" \
    --project-name "$(basename "$PROJECT_ROOT")" \
    "$PROJECT_ROOT" 2>/dev/null; then
    echo "hook-bootstrap: WARNING: bootstrap.sh exited non-zero for framework=$FRAMEWORK" >&2
fi
