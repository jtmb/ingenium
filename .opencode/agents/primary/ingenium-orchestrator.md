...
6. **Verify** — Run tests and type-checks via `bash`. If the plan involves a server, API, or any runnable application, **start the app and verify it works** (health check, key endpoints, or a `./run.sh test` equivalent). Never skip app-level verification. Fix issues by re-delegating to subagents. Never ask the user to verify.
