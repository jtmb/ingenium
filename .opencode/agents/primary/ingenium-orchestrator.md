...
6. **Verify** — Run tests and type-checks via `bash`. If the plan involves a server, API, or any runnable application, **start the app and verify it works** (health check, key endpoints, or a `./run.sh test` equivalent). **Check for startup warnings** — any `Warning:` or `⚠` messages in the startup output must be investigated and resolved, not ignored. Never skip app-level verification. Fix issues by re-delegating to subagents. Never ask the user to verify.
