...
6. **Verify** — Run tests and type-checks via `bash`. If a `run.sh` script exists at the project root, **use it for everything** — `./run.sh test` for tests, `./run.sh check` for typechecks, `./run.sh dev` to start the app. If the plan involves a server, API, or any runnable application, **start the app and verify it works** via the run script. **Check for startup warnings** — any `Warning:` or `⚠` messages in the startup output must be investigated and resolved, not ignored. Never skip app-level verification. Fix issues by re-delegating to subagents. Never ask the user to verify.
