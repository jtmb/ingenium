...
- **Exact-two Todo allocation** — enumerate independent, dependency-ready `TodoWrite` items, select up to three concurrently, and assign exactly one pair of exactly two agents to each selected Todo in one parallel call. One, two, or three selected Todos use 2, 4, or 6 agents; fewer than three eligible Todos leave capacity unused. Never invent a Todo or add a third agent; pair responsibilities remain distinct and non-overlapping, while writer, territory, dependency, and review gates remain in force.
...
committing, inspect status, diff, and recent log, then stage only intended paths.
Use ordinary non-interactive Git for local commits and `gh` for GitHub pushes,
pull requests, and checks. Never commit unrelated changes, rewrite published
history, or force-push without explicit authorization.

## 🔴 HARD RULEs — Bounded Quality Gates

- After an implementation wave is finalized and its declared verification is complete, independent applicable `@ingenium-qa`, `@ingenium-security-auditor`, and `@ingenium-qa-vision` reviews share one post-wave phase when safe. Each implementation boundary receives exactly one QA report and, only when the task contract predeclares a changed security surface, at most one eligible security report; visual QA follows its final-UI boundary. Reviewers have no task-delegation authority, cannot spawn one another, and cannot reopen a closed task. If a review is blocked or not applicable, declare its unused active slot and concrete reason rather than manufacturing work. After an in-scope reviewer blocker is fixed, run only the named minimum targeted regression, then proceed directly to the declared deploy and acceptance steps; never rerun QA, security, visual QA, or any other reviewer, and never create a recursive reviewer handoff.
