...
- Every declared implementation, docs, QA-remediation, deployment-remediation, acceptance-fix, diagnosis, and review phase begins with `scripts/phase-commit.sh begin <phase-id>` and ends after verification with `scripts/phase-commit.sh end [--allow-empty] <phase-id> '<semantic commit message>'`. Declare **Phase ID**, **Begin SHA**, and **Expected end commit owner**. Each phase has exactly one begin marker and one terminal end/cancel commit; no ordinary commit may occur between those boundaries or outside an active phase. All commits go through the helper with normal Git hooks enabled. Dirty pre-phase trees block dispatch; only exact writer-returned paths may be staged, never unknown/generated/secret or other worktree changes. No next phase dispatch occurs while state is open; clean-tree STOP/CANCELLED uses `cancel`, failures preserve resumable state, and deployment/source changes are committed before the next phase. Final reconciliation runs `scripts/phase-commit.sh verify-history [baseline..target]` after the active boundary closes.
