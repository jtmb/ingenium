...
- Todo allocation is pair-based: enumerate the independent, dependency-ready `TodoWrite` items before dispatch, select up to three concurrently, and assign exactly one pair of exactly two agents to each selected Todo in one parallel call. One, two, or three selected Todos therefore use 2, 4, or 6 agents; if fewer than three are eligible, leave the remaining capacity unused. Pair members need distinct responsibilities, and the phase must still preserve the three-writer maximum, exclusive territories, dependency order, and review gates.
...
- After a finalized implementation wave and its declared verification, independent applicable QA, security, and visual QA reviews share one post-wave phase when safe. Each implementation boundary receives exactly one QA report and, only when the task contract predeclares a changed security surface, at most one eligible security report; applicable visual QA follows the final UI boundary. Reviewers have no task-delegation authority, cannot spawn one another, and cannot reopen a closed task. If a review is blocked or not applicable, record its unused slot and concrete reason. After an in-scope reviewer blocker is fixed, run only the named minimum targeted regression, then proceed directly to the declared deploy and acceptance steps; never rerun QA, security, visual QA, or any other reviewer, even when the remediation changes the reviewer’s declared boundary, and never create a recursive reviewer handoff.
...
1. Declare the causal task contract, phase counts/territories, dynamic read-only ceiling, dependencies, and unused capacity.
2. Enumerate and select up to three independent, dependency-ready `TodoWrite` items, then delegate one exactly-two-agent pair per selected Todo in one parallel call. Preserve non-overlapping responsibilities, the 6-active/3-writer limit, exclusive territories, dependency order, and review gates; if fewer than three Todos are eligible, leave capacity unused rather than inventing work or adding a third agent.
3. After implementation, run one shared post-wave phase for independent applicable QA, security, and visual review when safe; `@ingenium-qa` solely owns a declared full E2E/container suite.
...
Introduce the work in plain language, then show the structured contract. After each phase, explain the result, changed files, checks, findings, and next dependency in human terms before immediately continuing any open work. The terminal response summarizes status, changes, verification count, findings or remaining work, and links to retained proof; raw agent/tool output is not a substitute for interpretation.
