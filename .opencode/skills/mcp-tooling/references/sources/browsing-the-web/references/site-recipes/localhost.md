...
| Expired authenticated browser session | Repeated `401` responses from `/api/v1/auth/session` after deployment/reconnect | Stop before any gate interaction and request an authorized reauthentication; never submit or expose credentials through browser automation |
