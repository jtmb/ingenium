...
- **Save screenshots to the project's `/tmp/opencode/ingenium-acceptance/` directory** — This is the canonical location for UI test artifacts
...
await playwright_browser_take_screenshot({
  path: "/tmp/opencode/ingenium-acceptance/skills.png",
  type: "png",
  scale: "css",
  fullPage: true
});
...
**Screenshot Options:**
- `path`: Absolute or relative path to save the screenshot (use `/tmp/opencode/ingenium-acceptance/`)
...
```bash
ls -lhS /tmp/opencode/ingenium-acceptance/
```
...
```typescript
await playwright_browser_take_screenshot({
  path: "/tmp/opencode/ingenium-acceptance/projects.png",
  type: "png",
  scale: "css",
  fullPage: true
});
...
for (const page of pages) {
  ...
  // Take screenshot
  await playwright_browser_take_screenshot({
    path: `/tmp/opencode/ingenium-acceptance/${page.name}.png`,
    type: "png",
    scale: "css",
    fullPage: true
  });
...
await runCommand("ls -lhS /tmp/opencode/ingenium-acceptance/");
```
